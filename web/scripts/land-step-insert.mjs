...
// step, in the file's own indentation, with nothing else in the file touched. Give beforeStepId
// instead of afterStepId to write the step right before the named one, which is the only way to
// place a step first in a quest (added 26 September 2026 for rule 33's "Make it yours" steps). Ids are append-only
...
for (const [rel, list] of byFile) {
  ...
  for (const e of list) {
    ...
    if (Boolean(e.afterStepId) === Boolean(e.beforeStepId)) {
      throw new Error(`${e.step.id}: give exactly one of afterStepId or beforeStepId`);
    }
    const [start, end] = locateStep(text, e.afterStepId ?? e.beforeStepId);
    ...
    text = e.afterStepId
      ...
      : text.slice(0, start) + body + ",\n" + " ".repeat(indent) + text.slice(start);
    JSON.parse(text);
    landed++;
  }

...
// Responsive audit (23 September 2026): under a loaded shared machine (several agents' own dev
// servers, emulators and Playwright browsers all running at once in sibling C:\Work projects),
// the picker's own "Getting your trail ready..." wait after a profile tile click was measured
// taking up to ~40s here, not the few seconds it takes on a quiet machine -- so this used to time
// out and report a false "could not reach the profile" failure well before the app itself had
// actually failed anything. 10s was tuned for a quiet machine; 60s gives real slowness room
// without masking a genuine hang (this still throws, and still retries up to 3 times, if the
// picker truly never arrives).

...
const QUEST_ID = "s1-w05-build";
const quest = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "content", "seasons", "1", "weeks", "05", "build.json"), "utf8"));
const stepIndex = (id) => {
  const i = quest.steps.findIndex((s) => s.id === id);
  if (i < 0) throw new Error(`step ${id} not found in ${QUEST_ID}`);
  return i;
};
const TARGETS = [["w05-problem-notebook", stepIndex("s1-w05-build-16")]];

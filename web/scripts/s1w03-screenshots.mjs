...
const QUEST_ID = "s1-w03-build";
const quest = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "content", "seasons", "1", "weeks", "03", "build.json"), "utf8"));
const stepIndex = (id) => {
...
const TARGETS = [
  ["resistor", stepIndex("s1-w03-build-09")],
  ["breadboard-map", stepIndex("s1-w03-build-10")],
  ["good-place-to-stop", stepIndex("s1-w03-build-11")],
  ["why-0-means-pressed", stepIndex("s1-w03-build-12")],
];
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `s1w03-${label}-${viewport.name}.png`);

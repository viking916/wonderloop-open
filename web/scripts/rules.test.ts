...
describe("firestore rules: households and profile progress", () => {
  ...
  // 12 September 2026: Ask's saved transcripts (households/{hid}/profiles/{pid}/tutorChats/{id})
  // are not named anywhere in firestore.rules either -- covered by the same household wildcard,
  // the same way workings just above already proved. This guards that path the same way.
  it("a member reads and writes their household's own tutor chat documents", async () => {
    const alice = ctxFor("alice");
    const chatRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/tutorChats/s1-w01-think__s1-w01-think-01-p01`;
    await assertSucceeds(setDoc(doc(alice, chatRefPath), {
      questId: "s1-w01-think", stepId: "s1-w01-think-01", problemId: "s1-w01-think-01-p01",
      prompt: "What is 3/4 plus 1/4?", messages: [{ role: "child", text: "I do not get it", at: 1 }], updatedAt: 1,
    }));
    await assertSucceeds(getDoc(doc(alice, chatRefPath)));
  });

  it("a non-member is denied read and write on another household's tutor chat documents", async () => {
    const eve = ctxFor("eve");
    const chatRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/tutorChats/s1-w01-think__s1-w01-think-01-p01`;
    await assertFails(getDoc(doc(eve, chatRefPath)));
    await assertFails(setDoc(doc(eve, chatRefPath), {
      questId: "s1-w01-think", stepId: "s1-w01-think-01", problemId: "s1-w01-think-01-p01",
      prompt: "snooping", messages: [], updatedAt: 1,
    }));
  });

  // 25 September 2026 (A4, Build and Make Ask): a step chat lives in the same tutorChats
  // collection as a problem chat, at builderChatId's "__step__" id (lib/domain/builderAsk.ts),
  // covered by the same household wildcard -- this proves that directly rather than assuming it,
  // the same way the problem-chat pair just above already does.
  it("a member reads and writes their household's own Builder Ask step chat documents", async () => {
    const alice = ctxFor("alice");
    const stepChatRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/tutorChats/s1-w05-build__step__s1-w05-build-11`;
    await assertSucceeds(setDoc(doc(alice, stepChatRefPath), {
      questId: "s1-w05-build", stepId: "s1-w05-build-11", problemId: "s1-w05-build-11",
      prompt: "Connect mBot, step by step", kind: "step",
      messages: [{ role: "child", text: "it will not connect", at: 1 }], updatedAt: 1,
    }));
    await assertSucceeds(getDoc(doc(alice, stepChatRefPath)));
  });

  it("a non-member is denied read and write on another household's Builder Ask step chat documents", async () => {
    const eve = ctxFor("eve");
    const stepChatRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/tutorChats/s1-w05-build__step__s1-w05-build-11`;
    await assertFails(getDoc(doc(eve, stepChatRefPath)));
    await assertFails(setDoc(doc(eve, stepChatRefPath), {
      questId: "s1-w05-build", stepId: "s1-w05-build-11", problemId: "s1-w05-build-11",
      prompt: "snooping", kind: "step", messages: [], updatedAt: 1,
    }));
  });

  // 23 September 2026: the piano/chess practice checkboxes (households/{hid}/profiles/{pid}/
  // practice/{docId}) are not named anywhere in firestore.rules either -- covered by the same
  // household wildcard, the same way workings and tutorChats above already proved.
  it("a member reads and writes their household's own practice documents", async () => {
    const alice = ctxFor("alice");
    const practiceRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/practice/s1w01`;
    await assertSucceeds(setDoc(doc(alice, practiceRefPath), { piano: true, chessSat: true, updatedAt: 1 }));
    await assertSucceeds(getDoc(doc(alice, practiceRefPath)));
  });

  it("a non-member is denied read and write on another household's practice documents", async () => {
    const eve = ctxFor("eve");
    const practiceRefPath = `households/${HOUSEHOLD_A}/profiles/${PROFILE_A1}/practice/s1w01`;
    await assertFails(getDoc(doc(eve, practiceRefPath)));
    await assertFails(setDoc(doc(eve, practiceRefPath), { piano: true, updatedAt: 1 }));
  });

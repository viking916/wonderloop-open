#!/usr/bin/env node
// Photographs the parent landing page's "What you need, season by season" card (6 September
// 2026): the season picker with each season's cost, the arc total, and season 3's list.
//
//   node --env-file=.env.development.local scripts/shopping-screenshots.mjs
...
const SPEAK_UID = "shopping-check-uid";
const SPEAK_HID = "shopping-check-household";
const SPEAK_PID = "shopping-check-explorer";
const SPEAK_EMAIL = "shopping-check@wonderloop.test";
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return (
        getApps().find((a) => a.name === "shopping-check-admin") ??
        initializeApp({ projectId: "wonderloop-dev" }, "shopping-check-admin")
      );
    })();
  }
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `shopping-${label}-${viewport.name}.png`);
  await page.screenshot({ path: file, fullPage: true });
  console.log(`  saved ${file} (${fs.statSync(file).size} bytes)`);
}

async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) { console.error("shopping-screenshots: FIRESTORE_EMULATOR_HOST is not set."); return 1; }
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      await page.getByText("What you need, season by season").waitFor({ timeout: 20000 });
      await page.waitForTimeout(400);
      await shoot(page, "season-1", viewport);
      await page.getByRole("tab", { name: /Season 3/ }).click();
      await page.waitForTimeout(400);
      await shoot(page, "season-3", viewport);
      await page.close();
    }
  } finally {
    await teardownHousehold();

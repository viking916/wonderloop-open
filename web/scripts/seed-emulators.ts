...
async function main() {
  ...
  await upsert(`households/${HID}/profiles/${EXPLORER_PID}`, {
    ...
    // The Play track on, so screenshots (This Week's Practice this week card, lib/domain/
    // practice.ts) show the piano row. chessPractice is left unset, so the two chess checkboxes
    // show too -- chess defaults on for every Explorer profile.
    playTrack: true,
  });

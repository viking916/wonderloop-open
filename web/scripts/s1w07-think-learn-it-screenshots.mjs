...
const QUEST_ID = "s1-w07-think";
const quest = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "content", "seasons", "1", "weeks", "07", "think.json"), "utf8"));
const stepIndex = (id) => {
  const i = quest.steps.findIndex((s) => s.id === id);
  if (i < 0) throw new Error(`step ${id} not found in ${QUEST_ID}`);
  return i;
};
const TARGETS = [["w07-learn-1",stepIndex("s1-w07-think-07")],["w07-learn-2",stepIndex("s1-w07-think-08")],["w07-learn-3",stepIndex("s1-w07-think-09")]];
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `s1w07-think-${label}-${viewport.name}.png`);

 ...
 * baselining all 12 weeks x every step x every viewport would multiply the accept-new-baselines
...
const VISUAL_TARGETS = new Set([
  ...
  "activity-piles",
  // "parent-view" only ever captures the PIN gate itself (ParentPinGate's "Keep the parent view
  // private?" screen, shown because the seeded household has no PIN) -- styleguide 8: a screen
  // behind a gesture is not covered until something performs the gesture, and pressing "Not now"
  // is exactly that gesture. The three targets below are the restructured Parent view (15
  // September 2026: a title area plus a This week/Season plan/To review/Shopping/Settings
  // tablist) reached immediately after it, added here because the review that found this gap
  // named them by name -- see the Parent section of runAllViewports for where "Not now" is
  // pressed and each tab reached. To review and Shopping are not covered here; that was not
  // asked for and would multiply the accept-new-baselines surface the header note above already
  // warns against.
  "parent-view",
  "parent-week",
  "parent-plan",
...
const VIEWPORTS = [
  ...
  // 15 September 2026: nothing here was ever wider than 1280, so every screen capped itself at
  // 720 to 1240px and a real laptop showed a small centred card in a field of kraft, which no
  // gate could see (owner: "tablet style constrained UI on laptop view"). 1536x864 is the most
  // common Windows laptop CSS viewport at 125% scaling; 1920x1080 is a full HD screen at 100%.
  { name: "1536x864", width: 1536, height: 864 },
  { name: "1920x1080", width: 1920, height: 1080 },
  // Responsive audit (23 September 2026): the owner reported the laptop still looked tablet-like
  // after the 15 September fix. Root cause: every laptop-width breakpoint in app/globals.css
  // (the problem screen's two-column split, the five-track-card row, the skills grid's fourth
  // column) waited for min-width 1281px or 1400px, both wider than a real laptop's own CSS
  // viewport -- 1366x768 and 1280x720 are two of the most common Windows laptop panel/scaling
  // combinations (a 1920 or 1600px physical panel at 125%/150% scaling), and neither 1536x864 nor
  // 1920x1080 above ever exercised anything narrower than 1400px. Added so a regression on either
  // breakpoint at the width that actually reaches most laptops fails this gate the same way the
  // 15 September fix's own two viewports do.
...
/**
 * Waits for the seeded Explorer's real progress to reach the Parent view's default "This week"
 * tab, never for a duration. scripts/parent-tabs-screenshots.mjs found this the hard way, with a
 * fine-grained (100ms-tick) sample loop: a capture once showed "Nothing done yet" and every
 * quest "Not started" even though the Explorer's own This Week screen for the same household
 * correctly showed "Build done" -- a real race, not a page bug. waitForSettled above only waits
 * for the page-level loading spinner and route-ready markers; the Parent view's progress comes
 * from a separate Firestore onSnapshot (lib/data/progress.ts's watchProgress) that starts only
 * once ParentHome mounts and resolved, in that script's own reproduction, roughly 100-120ms after
 * the tab already looked settled by that check alone.
 */
async function waitForParentWeekData(page) {
  await page.getByText("Build done", { exact: true }).waitFor({ timeout: 15000 });
}

/**
 * Waits for the Season plan tab to actually render a week row (components/parent/SeasonIndex.tsx's
 * <li id="season-week-N" class="pr-season-row">), never for a duration. SeasonIndex builds its
 * table from the same progress waitForParentWeekData above already waited for, but the table
 * itself is a separate render tree that only mounts once the "Season plan" tab is selected, so
 * that earlier wait does not by itself prove this one is ready.
 */
async function waitForSeasonPlanRow(page) {
  await page.locator(".pr-season-row").first().waitFor({ timeout: 15000 });
}
...
async function visitAndCheck(page, route, viewportName, allFindings, consoleErrors) {
  ...
  // A route that lands on a problem step (quest-resume, quest-stepN) has a second loading gate
  // of its own after the page-level one: ProblemPlayer's "Getting this problem ready..." wait,
  // which clears only once its attempts listener and QuestShell's progress listener have both
  // delivered. Only the two scripted interactions used to wait for it, so quest-resume at
  // 390x844 captured, and enshrined as a baseline, a screen with no answer options and no Check
  // (styleguide section 8: check that a capture shows the state its label claims). Cheap because
  // it only waits when that element is actually on the page.
  if ((await page.locator(".tr-problem__wait").count()) > 0) {
 ...
 * re-navigation. Review finding 1: the app's per-week content (the monster card -- shown for a
 * whole sprint, weeks 1-4/5-8/9-12, since A1 (2026-09-24), not just its own authored week -- the
 * TrackCards, the season book) genuinely differs, and this control is the only way any of it is
 * ever reachable from "This Week".
 */
 ...
 */
// Responsive audit (23 September 2026): on a loaded shared machine (several agents' own dev
// servers, emulators and Playwright browsers all running at once in sibling C:\Work projects),
// the picker's post-click wait was measured taking up to ~40s here, well past the 10s this was
// tuned for on a quiet machine, which made this gate fail with a false "crashed" before the app
...
async function selectProfile(page, kind) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      await page.waitForURL((url) => url.pathname.startsWith(`/${kind}`), { timeout: 60000 });
      await waitForSettled(page);
      return;
    } catch (err) {
...
async function runAllViewports(browser, session, allFindings, log) {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    // Parent tabs (gap found in review, 15 September 2026): the route above only ever captures
    // ParentPinGate's own "Keep the parent view private?" prompt -- the seeded household has no
    // PIN, so the restructured tabbed Parent view behind it (app/parent/page.tsx: a title area
    // plus a This week/Season plan/To review/Shopping/Settings tablist, tab kept in the URL hash)
    // had never once been reached by this gate. Dismiss the gate the same real way a parent would
    // (mechanics borrowed from scripts/parent-tabs-screenshots.mjs, written for a one-off UI
    // review pass of the same restructure), then run the full check suite -- contrast, overflow,
    // console errors, button colour, link underline, and the visual-baseline compare -- against
    // three of its tabs as real, named screens, not just a demo screenshot.
    {
      ...
      // "This week" is the default active tab on a fresh load. See waitForParentWeekData's own
      // comment for why waitForSettled alone is not enough here.
      consoleErrors.length = 0;
      await waitForSettled(page);
      await waitForParentWeekData(page);
      const rWeek = await checkCurrentPage(page, "parent-week", viewport.name, allFindings, consoleErrors);
      log.push(`${viewport.name} parent/parent-week: ${rWeek.findingCount} finding(s), ${rWeek.screenshot}`);

      // Season plan: a real tab click (never a re-navigation), then a real week row rather than a
      // duration -- see waitForSeasonPlanRow's own comment.
      consoleErrors.length = 0;
      await page.getByRole("tab", { name: "Season plan", exact: true }).click({ timeout: 10000 });
      await waitForSettled(page);
      await waitForSeasonPlanRow(page);
      const rPlan = await checkCurrentPage(page, "parent-plan", viewport.name, allFindings, consoleErrors);
      log.push(`${viewport.name} parent/parent-plan: ${rPlan.findingCount} finding(s), ${rPlan.screenshot}`);

      // Settings: nothing here waits on a listener of its own beyond what waitForSettled already
      // covers -- InviteCard/ProfileManager/PinSettings/AiKeyCard/DeviceCheck all read from the
      // household doc that has been watched since the gate was dismissed above, and the selected
      // child's Resets need only the current-week value the "This week" wait above already proved
      // had arrived.
      consoleErrors.length = 0;
      await page.getByRole("tab", { name: "Settings", exact: true }).click({ timeout: 10000 });
      await waitForSettled(page);
      const rSettings = await checkCurrentPage(page, "parent-settings", viewport.name, allFindings, consoleErrors);
      log.push(`${viewport.name} parent/parent-settings: ${rSettings.findingCount} finding(s), ${rSettings.screenshot}`);
    }

#!/usr/bin/env node
// Photographs knowing-what-you-know (7 September 2026): the confidence chips before a check,
// the sure-and-wrong nudge with the miss-kind question after a first miss, and the calibration
// card on the skills map. Runs on season 1 week 5's warm-up.
//
//   node --env-file=.env.development.local scripts/calibration-screenshots.mjs
...
const SPEAK_UID = "calib-check-uid";
const SPEAK_HID = "calib-check-household";
const SPEAK_PID = "calib-check-explorer";
const SPEAK_EMAIL = "calib-check@wonderloop.test";
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return getApps().find((a) => a.name === "calib-check-admin") ?? initializeApp({ projectId: "wonderloop-dev" }, "calib-check-admin");
    })();
  }
...
async function ensureAllowlistedIdentity(uid, email) {
  ...
  await db.doc(`allowedEmails/${email}`).set({ note: "calibration screenshot identity", addedAt: Date.now() });
}
...
async function createHousehold() {
  const db = await getDb();
  await db.doc(`users/${SPEAK_UID}`).set({ displayName: "Calibration Check", householdIds: [SPEAK_HID] });
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `calibration-${label}-${viewport.name}.png`);
  await page.screenshot({ path: file, fullPage: true });
  console.log(`  saved ${file} (${fs.statSync(file).size} bytes)`);
}

async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) { console.error("calibration-screenshots: FIRESTORE_EMULATOR_HOST is not set."); return 1; }
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      await waitForSettled(page);
      await page.getByRole("button", { name: "Sure" }).waitFor({ timeout: 20000 });
      await page.waitForTimeout(300);
      await shoot(page, "chips", viewport);
      await page.getByRole("button", { name: "Sure" }).click();
      await page.getByRole("textbox").first().fill("1");
      await page.getByRole("button", { name: "Check" }).click();
      await page.getByText("What kind of miss was that?").waitFor({ timeout: 20000 });
      await page.waitForTimeout(400);
      await shoot(page, "miss", viewport);
      await page.getByRole("button", { name: "I misread it" }).click();
      await page.waitForTimeout(800);
      console.log(`  sure, wrong, misread recorded (${viewport.name})`);

      await page.goto(freshUrl("/explorer/skills"), { waitUntil: "load" });
      await waitForSettled(page);
      await page.getByText("Knowing what you know").waitFor({ timeout: 20000 });
      await page.waitForTimeout(400);

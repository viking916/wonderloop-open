#!/usr/bin/env node
// One-off verification script for the Speak recording fix (components/quest/ArtifactStep.tsx):
// open a recording-only Speak artifact step in the real running app, screenshot it, mark the
// recording made, finish the Speak log, and screenshot This Week and the portfolio so the whole
// path -- unfinishable step to a quest that actually reads "done" -- is looked at, not assumed.
//
// Writes go to a household this script creates and deletes again (SPEAK_HID below), never the
// seeded "home" household: the owner checklist's visual baselines are captured from "home", and
// a leftover done quest in it would show up as a visual regression in an unrelated gate run.
// Mirrors the sign-in and isolated-household approach in scripts/verify-ui.mjs.
//
// Run with the emulators up and `npm run dev` serving port 3000:
//   node --env-file=.env.development.local scripts/speak-recording-screenshots.mjs
...
const SPEAK_UID = "speak-recording-uid";
const SPEAK_HID = "speak-recording-household";
const SPEAK_PID = "speak-recording-explorer";
const SPEAK_EMAIL = "speak-recording@wonderloop.test";
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return (
        getApps().find((a) => a.name === "speak-recording-admin") ??
        initializeApp({ projectId: "wonderloop-dev" }, "speak-recording-admin")
      );
    })();
  }
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `speak-recording-${label}-${viewport.name}.png`);
...
async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) {
    console.error("speak-recording-screenshots: refusing to run -- FIRESTORE_EMULATOR_HOST is not set. Run via node --env-file=.env.development.local.");
    return 1;
  }
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      // 1. The step as he first meets it.
      await page.goto(freshUrl(`/explorer/quest/${QUEST_ID}?step=${ARTIFACT_STEP}`), { waitUntil: "load" });
      await waitForSettled(page);
      await shoot(page, "step-before", viewport);

      // 2. Marked, by the kept-elsewhere path (components/quest/RecordingBlock.tsx): headless
      // Chromium has no camera, so the in-app Record path is not what this script exercises.
      await page.getByRole("button", { name: "I recorded it somewhere else" }).click({ timeout: 10000 });
      await page.waitForSelector("text=Marked as made", { timeout: 10000 });
      await page.waitForTimeout(400);
      await shoot(page, "step-after", viewport);

      // 3. The Speak log, so the quest can actually reach done (recording-and-log).
      ...
      await shoot(page, "log-saved", viewport);

      // 4. This Week: the Speak card's own status.
      await page.goto(freshUrl("/explorer"), { waitUntil: "load" });
      await waitForSettled(page);
      await shoot(page, "this-week", viewport);

      // 5. The portfolio spread for a quest whose recording is on the phone.

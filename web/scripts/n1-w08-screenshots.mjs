...
const QUEST_ID = "s1-w08-build";
const quest = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "content", "seasons", "1", "weeks", "08", "build.json"), "utf8"));
const stepIndex = (id) => {
  const i = quest.steps.findIndex((s) => s.id === id);
  if (i < 0) throw new Error(`step ${id} not found in ${QUEST_ID}`);
  return i;
};
const TARGETS = [["w08-make-it-yours", stepIndex("s1-w08-build-05")]];

#!/usr/bin/env node
// Photographs a quest's extras view (6 September 2026): the "If there is time" rail entry, the
// bonus tracks and the make-your-own brief, and one extra ticked. Quest id on the command line,
// default s1-w01-build.
//
//   node --env-file=.env.development.local scripts/extras-screenshots.mjs [questId]
...
const SPEAK_UID = "extras-check-uid";
const SPEAK_HID = "extras-check-household";
const SPEAK_PID = "extras-check-explorer";
const SPEAK_EMAIL = "extras-check@wonderloop.test";

const QUEST_ID = process.argv[2] || "s1-w01-build";
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return (
        getApps().find((a) => a.name === "extras-check-admin") ??
        initializeApp({ projectId: "wonderloop-dev" }, "extras-check-admin")
      );
    })();
  }
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `extras-${label}-${viewport.name}.png`);
  await page.screenshot({ path: file, fullPage: true });
  console.log(`  saved ${file} (${fs.statSync(file).size} bytes)`);
}

async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) { console.error("extras-screenshots: FIRESTORE_EMULATOR_HOST is not set."); return 1; }
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      await page.goto(freshUrl(`/explorer/quest/${QUEST_ID}?step=extras`), { waitUntil: "load" });
      await waitForSettled(page);
      await page.getByRole("heading", { name: "If there is time" }).waitFor({ timeout: 20000 });
      await page.waitForTimeout(500);
      await shoot(page, "list", viewport);
      await page.getByRole("button", { name: "I did this" }).first().click();
      await page.getByText("Done.").first().waitFor({ timeout: 15000 });
      await page.getByText("1 of").waitFor({ timeout: 15000 });
      console.log(`  first extra ticked and counted in the rail (${viewport.name})`);
      await page.waitForTimeout(400);
      await shoot(page, "ticked", viewport);
      // Tick the rest too, then look at the idea box: a ticked extra that names an idea meets it.
      while (await page.getByRole("button", { name: "I did this" }).count()) {
        await page.getByRole("button", { name: "I did this" }).first().click();
        await page.waitForTimeout(600);
      }
      await page.goto(freshUrl("/explorer/skills"), { waitUntil: "load" });
      await waitForSettled(page);
      await page.getByRole("heading", { name: "Idea box" }).waitFor({ timeout: 20000 });
      await page.waitForTimeout(800);
      await shoot(page, "skills-ideas", viewport);
      await page.goto(freshUrl(`/explorer/quest/${QUEST_ID}?step=extras`), { waitUntil: "load" });
      await waitForSettled(page);
      await page.getByRole("button", { name: "Back to the steps" }).click();
      await page.waitForURL((url) => /step=\d+/.test(url.search), { timeout: 15000 });
      console.log("  Back to the steps returned to a numbered step");
      await page.close();
    }
  } finally {
    await teardownHousehold();

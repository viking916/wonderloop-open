#!/usr/bin/env node
// Reproduces the week 1 Build artifact step's "Save code" path in the real running app (the
// owner reported a failed save on the first real session, 5 September 2026): pastes a MakeCode
// share link, presses Save code, and photographs the result, printing any route or console
// error and any failure toast. Also photographs the quest screen's way home.
//
//   node --env-file=.env.development.local scripts/codesave-screenshots.mjs
...
const SPEAK_UID = "codesave-check-uid";
const SPEAK_HID = "codesave-check-household";
const SPEAK_PID = "codesave-check-explorer";
const SPEAK_EMAIL = "codesave-check@wonderloop.test";
...
const ARTIFACT_STEP_ID = buildQuest.steps[ARTIFACT_STEP].id;
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return (
        getApps().find((a) => a.name === "codesave-check-admin") ??
        initializeApp({ projectId: "wonderloop-dev" }, "codesave-check-admin")
      );
    })();
  }
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `codesave-${label}-${viewport.name}.png`);
  await page.screenshot({ path: file, fullPage: true });
  console.log(`  saved ${file} (${fs.statSync(file).size} bytes)`);
}

async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) { console.error("codesave-screenshots: FIRESTORE_EMULATOR_HOST is not set."); return 1; }
  ...
  try {
    for (const viewport of VIEWPORTS) {
      ...
      page.on("response", (res) => { if (res.status() >= 400 && !res.url().includes("_next")) console.log(`  [http ${res.status()}] ${res.url().slice(0, 120)}`); });
      await signIn(page, bundle, token);
      await selectProfile(page);

      // The quest screen's way home, on the first step.
      await page.goto(freshUrl(`/explorer/quest/${QUEST_ID}?step=0`), { waitUntil: "load" });
      await waitForSettled(page);
      await page.waitForSelector("text=Back to This Week", { timeout: 15000 });
      await shoot(page, "way-home", viewport);

      // Save code on the artifact step.
      await page.goto(freshUrl(`/explorer/quest/${QUEST_ID}?step=${ARTIFACT_STEP}`), { waitUntil: "load" });
      await waitForSettled(page);
      await page.locator(`#${ARTIFACT_STEP_ID}-code`).fill("https://makecode.microbit.org/_Xk4Ac2LfEUPq");
      await page.getByRole("button", { name: "Save code" }).click({ timeout: 10000 });
      const outcome = await Promise.race([
        page.waitForSelector("text=Saved: code", { timeout: 20000 }).then(() => "saved"),
        page.waitForSelector("text=/did not save/", { timeout: 20000 }).then(() => "failed"),
      ]).catch(() => "neither");
      console.log(`  save code outcome (${viewport.name}): ${outcome}`);
      if (outcome === "failed") console.log("  toast: " + (await page.locator("text=/did not save/").first().innerText()));
      await page.waitForTimeout(400);
      await shoot(page, "save-code", viewport);

      // Back to This Week through the new link.
      await page.getByRole("link", { name: "Back to This Week" }).click({ timeout: 10000 });
      await page.waitForURL((url) => url.pathname === "/explorer", { timeout: 15000 });
      console.log("  Back to This Week navigated to /explorer");
      await page.close();
    }
  } finally {
    await teardownHousehold();

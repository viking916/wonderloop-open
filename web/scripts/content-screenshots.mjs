#!/usr/bin/env node
// Photographs any quest step, for a throwaway profile placed in a given season and week, in the
// real running app. Used to look at authored content changes by eye before they are called done
// (docs/ui-styleguide.md section 7): the UI gate only ever photographs season 1 week 1.
//
// Targets come from a JSON file: an array of { label, seasonId, week, questId, stepIndex }.
// Screenshots land in docs/screenshots/content-<label>-<viewport>.png. The household is created
// and deleted again, never the seeded "home" household.
//
//   node --env-file=.env.development.local scripts/content-screenshots.mjs targets.json
...
const SPEAK_UID = "content-check-uid";
const SPEAK_HID = "content-check-household";
const SPEAK_PID = "content-check-explorer";
const SPEAK_EMAIL = "content-check@wonderloop.test";
...
async function getAdminApp() {
  if (!adminAppPromise) {
    adminAppPromise = (async () => {
      ...
      return (
        getApps().find((a) => a.name === "content-check-admin") ??
        initializeApp({ projectId: "wonderloop-dev" }, "content-check-admin")
      );
    })();
  }
...
async function shoot(page, label, viewport) {
  const file = path.join(SCREENSHOT_DIR, `content-${label}-${viewport.name}.png`);
...
async function run() {
  if (!process.env.FIRESTORE_EMULATOR_HOST) {
    console.error("content-screenshots: refusing to run -- FIRESTORE_EMULATOR_HOST is not set.");
    return 1;
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      for (const viewport of VIEWPORTS) {
        ...
        if (target.problemId) {
          // Mark every problem before the wanted one in this step as answered, so the player
          // opens on the wanted problem rather than the step's first.
          const db = await getDb();
          const questFile = path.join(REPO_ROOT, "content", "seasons", String(target.seasonId), "weeks", String(target.week).padStart(2, "0"), target.questId.split("-").pop() + ".json");
          const quest = JSON.parse(fs.readFileSync(questFile, "utf8"));
          const step = quest.steps[target.stepIndex];
          const ids = (step.problems ?? []).map((pr) => pr.id);
          const before = ids.slice(0, ids.indexOf(target.problemId));
          if (before.length) {
            const outcomes = Object.fromEntries(before.map((id) => [id, "correct"]));
            await db.doc(`households/${SPEAK_HID}/profiles/${SPEAK_PID}/progress/${target.questId}`).set({
              questId: target.questId,
              status: "in_progress",
              stepIndex: target.stepIndex,
              quest: { ticks: [], checklist: {}, problemOutcomes: outcomes, explains: {}, artifacts: {}, logs: [], debates: [], lessons: [] },
              problems: {},
            }, { merge: true });
          }
        }
        ...
        await page.waitForTimeout(400);
        await shoot(page, target.label, viewport);
        await page.close();
      }
    }
  } finally {

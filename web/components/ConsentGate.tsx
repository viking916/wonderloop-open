 ...
 *
 * useAsyncAction (not Button's own automatic tracking) because the same `pending` value also
 * needs to disable the tick and the "Sign out" way out while the household is being created, not
 * only drive the primary button's own spinner.
 */
export function ConsentGate() {
  const { user, acceptTerms, signOut, error } = useSession();
  const [ticked, setTicked] = useState(false);
  const { run: handleContinue, pending } = useAsyncAction(acceptTerms);
      ...
      <Card tone="surface" shadow className="tr-signin__card tr-consent">
        ...
        <div className="tr-step__actions">
          <Button
            ...
            pending={pending}
            pendingLabel="Setting up your family's space"
            disabled={!ticked}
          >
            Create our family’s space
          </Button>

...
export function DataStep({ step, progress, onUpdate, onOwnPrimaryChange }: DataStepProps) {
  ...
  // meetsRequirements is the genuine teaching gate (enough rows, a real answer) -- the only
  // reason this control should ever show the disabled kraft fill. canSave folds in !saving too,
  // kept for primary/variant/the handleSave guard exactly as before; the button's own `disabled`
  // below reads meetsRequirements alone, so a mid-save moment shows the shared pending spinner
  // instead of the washed-out disabled fill (owner report: a disabled Check/Save looks broken).
  const meetsRequirements = completeRows.length >= step.minRows && answer.trim().length > 0;
  const canSave = meetsRequirements && !saving;
  ...
  return (
    <section className="tr-step" aria-labelledby={`${step.id}-title`}>
      ...
      />
      <div className="tr-step__actions">
        <Button variant={primary ? "primary" : "secondary"} onClick={handleSave} disabled={!meetsRequirements} pending={saving} pendingLabel="Saving the data">
          {saved ? "Saved" : "Save the data"}
        </Button>
      </div>
    </section>
  );

...
export type PhotoCaptureProps = {
  ...
  idPrefix: string;
  /** True while the parent's own onPhoto (the Storage upload) is in flight. Blocks every OTHER
   * control here for real (retaking or picking a different file mid-upload would abandon or race
   * that upload), so those keep the plain `disabled` attribute; "Use this photo" is the one
   * control that itself starts that upload, so it gets `pending` instead (see its own onClick
   * below) -- the shared spinner, not the flat disabled fill, on the control actually working. */
  disabled?: boolean;
...
export function PhotoCapture({ idPrefix, disabled, onPhoto }: PhotoCaptureProps) {
  ...
  // Named confirmSnap, not "use" (react-hooks/rules-of-hooks reads any function named `use*`
  // as a Hook call, even a plain async helper like this one, once it is invoked from a
  // separately named function rather than an inline arrow). Returns its own Promise (never
  // wrapped in `void`) so the "Use this photo" Button below can track it automatically -- see
  // the pending/disabled split in the type doc above.
    ...
    <div className="tr-photo">
      ...
      {phase === "snapped" && snapUrl ? (
        <div className="tr-photo__live">
          ...
          <div className="tr-step__actions">
            <Button variant="primary" onClick={confirmSnap} pending={disabled} pendingLabel="Saving your photo">
              Use this photo
            </Button>

...
export type LessonPlayerProps = {
   ...
   * artifact step already does; may return that write's own Promise, which handleNext passes
   * straight back out of its own onClick so the last beat's "Done" button shows the shared
   * pending spinner for exactly as long as that write is in flight (see Button.tsx's automatic
   * Promise tracking). The struggle-offer modal (ProblemPlayer.tsx) just closes the overlay: it
   * never persists anything, on purpose (see the module doc below), and returns nothing.
   */
...
export function LessonPlayer({ lesson, onDone, onOwnPrimaryChange }: LessonPlayerProps) {
  ...
  function handleNext() {
    ...
    if (isLast) {
      // Returned, not just called: lets the "Done" button below track this as its own pending
      // Promise (Button.tsx's automatic tracking) when the caller's onDone does real work
      // (LessonStep.tsx's markDone write), same as any other Promise-returning onClick.
      return onDone();
    }
      ...
      {right ? (
        <>
          ...
          <div className="tr-step__actions">
            <Button variant="primary" onClick={handleNext} pendingLabel="Saving">

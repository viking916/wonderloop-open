...
export function InstructionStep({ step, progress, onUpdate, onOwnPrimaryChange }: InstructionStepProps) {
  ...
  return (
    <section className="tr-step" aria-labelledby={`${step.id}-title`}>
      ...
      <StepBody text={step.body} figure={step.figure ? <ProblemFigure figure={step.figure} /> : undefined} />
      <div className="tr-step__actions">
        {/* Polish pass (Package B): a completed step used to keep showing "Got it, done" as a
            disabled primary button, which reads as broken, not finished (styleguide 1: "prefer a
            pressable control with a helpful message over a disabled one"; caught on a revisited
            Build quest). Done now reads as a positive confirmation line, the same check-mark
            language ExplanationPanel already uses for a correct answer, instead of a dead-looking
            control with nothing left to press. */}
        {done ? (
          <p className="tr-step__confirm">
          ...
          <Button variant="primary" onClick={handleGotIt} pending={saving} pendingLabel="Saving">
            Got it
          </Button>
        )}
      </div>

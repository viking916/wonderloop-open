...
export function DebateRoom({ step, quest, progress, householdId, profileId, onUpdate, onOwnPrimaryChange }: DebateRoomProps) {
  ...
  // draftReady also requires busy === "idle": used for primaryLive below and for the buttons
  // that are NOT themselves the one currently in flight (Keep it and move on can run while a
  // Send is not, but must not fire while one already is). The Send buttons themselves gate on
  // hasDraft alone and carry `pending` instead, so a real network wait never shows the flat
  // disabled fill on the control actually doing the waiting (owner report: a disabled control
  // looks broken, not busy).
  const draftReady = hasDraft && busy === "idle";
  const primaryLive =
    !stepDone &&
    busy === "idle" &&
      ...
      <section className="tr-step tr-debate" aria-labelledby={`${step.id}-title`}>
        ...
        {stepDone ? (
          ...
          <div className="tr-step__actions">
            <Button variant="primary" onClick={markDebatedOutLoud} pending={markingOffline} pendingLabel="Saving">
              We debated it out loud
            </Button>
          </div>
        )}
    ...
    <section className="tr-step tr-debate" aria-labelledby={`${step.id}-title`}>
      ...
      {phase === "side" ? (
        <div className="tr-debate__side-pick">
          ...
          <Button variant="primary" onClick={() => chooseSide("for")}>For</Button>
          <Button variant="secondary" onClick={() => chooseSide("against")}>Against</Button>
        </div>
      ) : null}
      ...
      {phase === "steelman" ? (
        <>
          ...
          <div className="tr-step__actions">
            <Button variant="primary" onClick={sendSteelman} disabled={!hasDraft} pending={busy === "sending"} pendingLabel={`Sending to ${OPPONENT}`}>
              {`Send to ${OPPONENT}`}
            </Button>
            {talkControl}
            {steelmanNote ? (
              <Button variant="quiet" onClick={keepSteelmanAnyway} disabled={!draftReady}>Keep it and move on</Button>
            ) : null}
          </div>
        </>
      ) : null}
      ...
      {round !== undefined ? (
        pending ? (
          busy === "idle" ? (
            <div className="tr-step__actions">
              <Button variant="primary" onClick={() => sendRound(pending)}>Try again</Button>
              <Button variant="quiet" onClick={sayItDifferently}>Say it differently</Button>
            </div>
          ) : null
        ) : (
          <>
            ...
            <div className="tr-step__actions">
              <Button
                variant="primary"
                onClick={() => sendRound(draft.trim())}
                disabled={!hasDraft}
                pending={busy === "sending"}
                pendingLabel={`Sending to ${OPPONENT}`}
              >
                {`Send to ${OPPONENT}`}
              </Button>
              {talkControl}
      ...
      {phase === "coach" ? (
        busy === "coach" ? (
          ...
          <div className="tr-step__actions">
            <Button variant="primary" onClick={requestCoach}>Get the coach card</Button>
          </div>
        )
      ) : null}

  ...
  return (
    <div className="tr-artifact-block tr-recording">
      ...
      {rec.status === "recorded" && rec.blob ? (
        discardAsked ? (
          ...
          <div className="tr-step__actions">
            <Button
              variant="primary"
              onClick={() => rec.blob && save(rec.blob, rec.mimeType ?? rec.blob.type)}
              pending={saving}
              pendingLabel="Saving your recording"
            >
              {copy.save}
            </Button>
      ...
      {onMarkMade && !saved && rec.status !== "recording" && rec.status !== "recorded" && !saving ? (
        <div className="tr-step__actions">
          <Button variant="quiet" onClick={handleMark} pending={marking} pendingLabel="Marking">
            I recorded it somewhere else
          </Button>
        </div>
      ) : null}
    </div>

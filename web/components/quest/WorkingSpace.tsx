...
export function WorkingSpace({ householdId, profileId, problemId, onContentChange, eagerLoad = false }: WorkingSpaceProps) {
  ...
  // The debounced write itself, not the keystroke/stroke that scheduled it -- "zero ceremony: no
  // save button" (task 43) stays true (nothing here is gated on this), but the owner's report
  // ("nothing happens instantly so it feels broken") applies just as much to a silent
  // background save: autoSaving flips true only once the write actually starts, and
  // useDelayedPending (the same shared anti-flicker timing Button.tsx and useAsyncAction use)
  // keeps a normal, fast save from flickering a "Saving" line on and off.
  const [autoSaving, setAutoSaving] = useState(false);
  const savingVisible = useDelayedPending(autoSaving);
  ...
  function commit(next: WorkingContent) {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      if (!data) return;
      setAutoSaving(true);
      void saveWorking(householdId, profileId, problemId, data).finally(() => setAutoSaving(false));
    }, SAVE_DEBOUNCE_MS);
  }
  ...
  return (
    <div className="tr-working">
      <Button variant="quiet" className="tr-working__toggle" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
      ...
      {open ? (
        <div className="tr-working__panel">
          {!loaded ? (
            ...
            <>
              <p className="tr-working__hint" aria-live="polite">
                {savingVisible ? "Saving..." : "Work it out here. Nobody grades this. It saves by itself."}
              </p>

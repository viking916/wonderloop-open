...
import { StepList, stepLabel } from "./StepList";
...
import { BuilderAskPanel } from "./BuilderAskPanel";
import { getContent } from "@/lib/content/app-content";
import { TRACK_LABEL, type Quest, type Step } from "@/lib/content/schema";
import { emptyQuestProgress, questStatus, resumePosition, type QuestProgress, type QuestStatus } from "@/lib/domain/completion";
import { builderAskAvailable, isBuilderAskStep } from "@/lib/domain/builderAsk";
...
export function QuestShell({ quest, profile, householdId }: QuestShellProps) {
  ...
  /**
   * Polish pass (Package B item 4, 15 September 2026): below 640px the whole side rail (quest
   * title, materials, six waypoints, extras, the done-for-now line and the stuck box) used to sit
   * ABOVE the current step in the DOM, so the actual question started about 900px down a phone
   * screen (styleguide 2: "everything the user needs to answer fits on one screen"). isMobile
   * drives an honest DOM reorder, not a CSS `order` trick: `order` only changes paint order, and
   * a keyboard/screen-reader user would still meet every side-rail control before the step's own
   * content, exactly backwards from what they see (styleguide 7's process rule -- "keyboard order
   * sensible" -- is failed by a purely visual reorder). Below, the side rail's own step/materials/
   * extras/park content moves into a native <details> AFTER the step content in the real DOM;
   * sideOpen mirrors its own open state so a manual toggle survives whatever re-renders this
   * screen (attempts loading, a save, the clock ticking) without this effect fighting it back
   * shut. Defaults to desktop (false) for SSR, where window is unavailable; corrected on mount
   * before paint is not possible without a flash, so a first paint at true phone width very
   * briefly shows the desktop shape until this effect runs -- the same one-render tradeoff
   * breakOffered/aiOn already make elsewhere in this file for state only the client can know.
   */
  ...
  // Polish pass (Package B, 15 September 2026): "Quest complete. Great work." used to fire from
  // plain status === "done", which is also true the instant a FINISHED quest is simply reopened
  // (Build week 1, revisited) -- a toast that fires on every load of an already-done quest reads
  // as new news when it is not. initialStatus captures the status the first moment real progress
  // has loaded (adjusted during render, guarded to run only once, the same pattern
  // prevStepIndexForPrimary above uses); justCompleted is true only when THIS visit watched the
  // quest cross from not-done into done, never when it was already done on arrival.
  const [initialStatus, setInitialStatus] = useState<QuestStatus | null>(null);
  if (progressLoaded && initialStatus === null) {
    setInitialStatus(status);
  }
  const justCompleted = initialStatus !== null && initialStatus !== "done" && status === "done";
  ...
  // Round 2 laptop-width fix (15 September 2026): a problem step manages its own, wider
  // two-column width (ProblemPlayer's .tr-problem__layout); every other step kind's body sits at
  // a 72ch measure (.tr-step__body), so this footer nav needs the matching .tr-quest__nav--measured
  // cap or "Continue" lands far to the right of the content it follows -- see app/globals.css's
  // own comment on both classes.
  const isProblemStep = step ? step.kind === "warmup" || step.kind === "problem-set" || step.kind === "puzzle-of-week" : false;

  // Polish pass (Package B item 4): the compact mobile header repeats the track/week eyebrow and
  // quest title the side rail's own heading already carries, plus names the step the sidebar's
  // StepList already names (stepLabel, now exported from StepList.tsx so both agree on wording
  // rather than drifting into two names for the same step).
  const trackWeekLabel = `${TRACK_LABEL[quest.track]} - Week ${quest.week}`;
  const stepProgressLabel = showExtras ? "Bonus tracks" : `Step ${stepIndex + 1} of ${quest.steps.length}`;
  const currentStepName = !showExtras && step ? stepLabel(step) : undefined;

  const sideHeader = (
    <>
      <span className="tr-eyebrow tr-quest__side-eyebrow">{trackWeekLabel}</span>
      <h2 className="tr-quest__title">{quest.title}</h2>
    </>
  );

  // Everything the side rail carries besides its own heading: the checklist, the waypoints, the
  // extras link, the way home and the stuck box. Package B item 4: below 640px this whole group
  // moves into a native <details> AFTER the step content instead of before it (see isMobile
  // above), so a child no longer scrolls past six steps and a materials list before ever reaching
  // the question.
  const sideBody = (
    <>
      ...
      <p className="tr-quest__home">
        {/* next/link, not a plain anchor: a full document load tears this page down before the
            route progress bar can paint, so the press looked like nothing happened (owner,
            16 September 2026). */}
        Done for now? <Link href="/explorer">Back to This Week</Link>. Everything you finished is saved.
      </p>
      {/* Polish pass (Package B): the icon-and-text row used to leave the button only the
          sidebar's own leftover width (icon plus gap plus padding), which was too narrow
          for its own label at this column's 250px width, so it wrapped to two lines and
          centred itself inside that narrow column (styleguide 2: "left-align it with the
          text above, no wrap"). The icon now sits beside the hint line only; the button
          gets the box's full inner width on its own row below, left-aligned under that
          hint, same as the text above it. */}
      <div className="tr-quest__park-box">
        <div className="tr-quest__park-head">
        ...
        <Button variant="quiet" className="tr-quest__park" onClick={handlePark}>
  ...
  const mainEl = (
    <div className="tr-quest__main">
      {justCompleted ? <Toast tone="success" message="Quest complete. Great work." /> : null}
      ...
      <p className="tr-eyebrow">{stepProgressLabel}</p>
      {showExtras ? (
        ...
        <>
          ...
          {/* A4 (year refinement 2026-09-24, owner ruling 0.1): "Stuck? Ask" on every Build and
              Make instruction, science, task and data step from season 1 week 5 on, at any time,
              no miss required -- a sibling to ProblemPlayer's own problem-scoped Ask, never a
              replacement for it. Both gates are pure (lib/domain/builderAsk.ts) so this renders
              nothing extra for a Think, Speak or Play quest, or for season 1 weeks 1 to 4. */}
          {builderAskAvailable(quest) && isBuilderAskStep(step) ? (
            <BuilderAskPanel householdId={householdId} profileId={profile.id} quest={quest} step={step} />
          ) : null}
        </>
      ) : (
      ...
      {showExtras ? null : (
      <>
        {/* Owner's real session, 12 September 2026: the last step of a done quest left a
            disabled "Continue" as the only visible forward control, next to a small toast
            and a tiny "Back to This Week" link buried in the left rail -- his child had no
            obvious way to leave. When an earlier step was skipped or parked instead, the
            same dead "Continue" gave no reason why and no way out either. This note only
            shows for that second case, right above the nav it explains. */}
        {isLastStep && !stepOwnPrimary && status !== "done" ? (
          <p className="tr-step__note">Some steps are still open. Tap one in the list to finish it.</p>
        ) : null}
        <div className={isProblemStep ? "tr-quest__nav" : "tr-quest__nav tr-quest__nav--measured"}>
          ...
          {/* Task 40 fix, generalized by task 41: while the step itself is already showing
              ...
              button, or ProblemPlayer's "Check"/"Next problem"/"Next step") this footer
              control is hidden rather than rendered alongside it, so only one primary
              control is ever on screen at a time (control language, commit 13b42c1: at most
              one primary per group). Once the step is done, its own control either
              disappears or (for the "Save..." family, which stays clickable so he can revise
              and resave) demotes to "secondary". */}
          {!stepOwnPrimary ? (
            isLastStep ? (
              status === "done" ? (
                /* 12 September 2026 defect: this is the quest's real finish line, so it
                   gets a real exit, not a disabled button. One primary way out (saved,
                   nothing left to do here) and one quiet way to see the result, same pair
                   shape as BreakOffer's own action row (.tr-break__actions: a plain flex
                   row with a gap, nothing break-specific about the class itself). */
                <div className="tr-break__actions">
                  <Button variant="primary" href="/explorer">
                    Done, back to This week
                  </Button>
                  <Button variant="quiet" href="/explorer/portfolio">
                    See it in the portfolio
                  </Button>
                </div>
              ) : (
                /* An earlier step was skipped or parked, so the quest is not actually done;
                   no primary control here (nothing to confirm), just a real way out. The
                   note above the nav says why "Continue" would have gone nowhere. */
                <Button variant="secondary" href="/explorer">
                  Back to This week
                </Button>
              )
            ) : (
              <button type="button" className="tr-btn tr-btn--primary" onClick={() => goToStep(stepIndex + 1)}>
                Continue
              </button>
            )
          ) : null}
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="tr-page">
        <Card tone="surface" shadow className="!p-0 tr-quest">
          {isMobile ? (
            <>
              {/* Package B item 4: the real DOM order at phone width, not a CSS `order` repaint --
                  see isMobile's own comment above for why a visual-only reorder would leave a
                  keyboard or screen-reader user meeting the whole side rail before the step it
                  belongs after. */}
              <div className="tr-quest__mobile-head">
                <span className="tr-eyebrow">{trackWeekLabel}</span>
                <h2 className="tr-quest__mobile-title">{quest.title}</h2>
                {/* Only the step's name: the step content's own eyebrow says "Step n of m" just
                    below, so repeating it here read twice on a phone. */}
                {currentStepName ? <p className="tr-quest__mobile-step">{currentStepName}</p> : null}
              </div>
              {mainEl}
              <details className="tr-quest__side-details" open={sideOpen} onToggle={(e) => setSideOpen(e.currentTarget.open)}>
                <summary className="tr-quest__side-summary">All steps</summary>
                <div className="tr-quest__side-inner">{sideBody}</div>
              </details>
            </>
          ) : (
            <>

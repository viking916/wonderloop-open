...
const mocks = vi.hoisted(() => ({
  ...
  setAttemptMissKind: vi.fn().mockResolvedValue(undefined),
  ...
  // "Ask" (12 September 2026): every ProblemPlayer instance checks /api/ai/health once per
  // mount (the same check DebateRoom.tsx already makes), regardless of whether any test below
  // cares about Ask -- defaulting to "not configured" here keeps every existing test's render
  // exactly as it was before Ask existed, and tests that DO care override this per test.
  health: vi.fn().mockResolvedValue({ ok: true, configured: false }),
}));
...
vi.mock("@/lib/data/progress", () => ({
  ...
  setAttemptMissKind: (...args: unknown[]) => mocks.setAttemptMissKind(...args),
...
vi.mock("@/lib/firebase/client", () => ({ getDb: () => ({}), getClientAuth: () => ({ currentUser: null }) }));
...
// "Ask"'s health check is the only thing in this file that calls the real global fetch; every
// other network-shaped call goes through the lib/data mocks above. Stubbing it once here, reset
// per test, means no existing test in this file needs to know Ask exists at all.
beforeEach(() => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async (input: unknown) => {
      const url = typeof input === "string" ? input : (input as Request).url;
      if (url.includes("/api/ai/health")) return { json: async () => mocks.health() } as Response;
      throw new Error(`unexpected fetch in ProblemPlayer.test.tsx: ${url}`);
    }),
  );
});
...
afterEach(() => {
  ...
  vi.unstubAllGlobals();
  vi.clearAllMocks();
  mocks.health.mockResolvedValue({ ok: true, configured: false });
});
...
// "Ask" (12 September 2026): "Still stuck? Ask" only shows once the child has missed at least
// once on this problem, this problem's hints are authored (the hintsAuthored prop, which
// LadderSession.tsx sets to false only for a stretch problem), and /api/ai/health reports the
// family's AI key configured. renderWithOneMiss reproduces one real wrong try the same way
// renderTwoWrongTries above reproduces two -- old enough (past the cooldown) that the button's
// gate, not a cooldown timer, is what each assertion below is actually testing.
describe("Ask (12 September 2026)", () => {
  function renderWithOneMiss(problem: Problem, hintsAuthored?: boolean) {
    const stepFor = { ...step, problems: [problem] } as Extract<Step, { kind: "problem-set" }>;
    const past = Date.now() - 300_000;
    const onUpdate = vi.fn().mockResolvedValue(undefined);
    render(
      <ProblemPlayer
        ...
        profileId="profile1"
        onUpdate={onUpdate}
        {...(hintsAuthored !== undefined ? { hintsAuthored } : {})}
      />,
    );
    act(() => {
      mocks.watchAttemptsCallbacks[0]([
  ...
  test("the button appears after one miss, once the family's AI key is configured", async () => {
    mocks.health.mockResolvedValue({ ok: true, configured: true });
    renderWithOneMiss(problem1);
    expect(await screen.findByRole("button", { name: "Still stuck? Ask" })).toBeTruthy();
  });

  test("the button never appears when this problem's hints are not authored (a Ladder stretch problem)", async () => {
    mocks.health.mockResolvedValue({ ok: true, configured: true });
    renderWithOneMiss(problem1, false);
    await screen.findByText("Hint 1"); // hydration finished and the hint panel is up
    expect(screen.queryByRole("button", { name: "Still stuck? Ask" })).toBeNull();
  });

  test("the button never appears when the family's AI key is not configured", async () => {
    mocks.health.mockResolvedValue({ ok: true, configured: false });
    renderWithOneMiss(problem1);
  ...
  test("the button never appears before any miss, even with hints authored and the key configured", async () => {
    mocks.health.mockResolvedValue({ ok: true, configured: true });
    renderPlayer();
...
// The "What kind of miss was that?" row (Knowing what you know, 7 September 2026) used to
// identify its attempt only from a local, in-session `lastAttemptId` set inside handleSubmit once
// persistAttempt's own promise resolved -- lost on a reload or a remount, and racy even within
// one session (see currentCycleFirstAttemptId's own doc comment, lib/domain/attempts.ts, and this
// suite's `renderWithOneMiss` above, which already showed a wrong try could be simulated purely
// via the attempts listener with no handleSubmit call at all -- something the row could not have
// survived before this fix). These tests drive the row the same way: through the listener alone,
// never through a real Check click, so a regression back to needing an in-session write fails
// them even though a full handleSubmit-driven test (see the other describe blocks above) would
// still pass.
describe("the 'What kind of miss was that?' row (currentCycleFirstAttemptId)", () => {
  function renderWithOneMissOnly(problem: Problem) {
    const stepFor = { ...step, problems: [problem] } as Extract<Step, { kind: "problem-set" }>;
    const past = Date.now() - 300_000;
    const onUpdate = vi.fn().mockResolvedValue(undefined);
    render(
    ...
    act(() => {
      mocks.watchAttemptsCallbacks[mocks.watchAttemptsCallbacks.length - 1]([
        {
          id: "attempt-from-firestore",
          attempt: {
            problemId: problem.id, questId: quest.id, answer: "wrong", correct: false, tryNumber: 1,
            hintTier: 0, ideaIds: [problem.ideaId], revealed: false, retry: false, at: past + 1000,
          },
  ...
  test("appears once the attempts listener alone reports a first-try miss, with no handleSubmit call at all", async () => {
    renderWithOneMissOnly(problem1);
    expect(await screen.findByRole("group", { name: "What kind of miss was that?" })).toBeTruthy();
    expect(mocks.persistAttempt).not.toHaveBeenCalled();
  });

  test("tapping a pill writes the miss kind onto the id the listener supplied, not a locally-invented one", async () => {
    renderWithOneMissOnly(problem1);
    const group = await screen.findByRole("group", { name: "What kind of miss was that?" });
    fireEvent.click(within(group).getByRole("button", { name: "I did not know the idea" }));

    expect(mocks.setAttemptMissKind).toHaveBeenCalledWith("hh1", "profile1", "attempt-from-firestore", "did-not-know");
  });

  test("survives a full unmount and remount (a reload), rebuilt from the same persisted attempt", async () => {
    // First mount: never even sees a wrong try, let alone sets any in-session state for it --
    // this stands in for the child reloading (or navigating back into) the quest step fresh,
    // with only the earlier session's persisted attempt waiting in Firestore.
    ...
    // Second mount: a fresh component instance, its own fresh watchAttemptsForQuest
    // subscription -- exactly like a real reload -- delivered the same attempt Firestore already
    // has on record.
    renderWithOneMissOnly(problem1);
    expect(await screen.findByRole("group", { name: "What kind of miss was that?" })).toBeTruthy();
  });

  test("does not appear once a second try is on record (view.triesUsed no longer 1), even though an attempt id exists", async () => {
    const stepFor = { ...step, problems: [problem1] } as Extract<Step, { kind: "problem-set" }>;
    const past = Date.now() - 300_000;
    render(
      <ProblemPlayer
        step={stepFor}
        quest={quest}
        progress={emptyQuestProgress()}
        problemsProgress={{ [problem1.id]: { firstSeenAt: past } }}
        progressLoaded={true}
        householdId="hh1"
        profileId="profile1"
        onUpdate={vi.fn().mockResolvedValue(undefined)}
      ...
      mocks.watchAttemptsCallbacks[0]([
        {
          id: "a1",
          attempt: {
            problemId: problem1.id, questId: quest.id, answer: "wrong", correct: false, tryNumber: 1,
            hintTier: 0, ideaIds: [problem1.ideaId], revealed: false, retry: false, at: past + 1000,
          },
        },
        {
          id: "a2",
          attempt: {
            problemId: problem1.id, questId: quest.id, answer: "wrong again", correct: false, tryNumber: 2,
            hintTier: 1, ideaIds: [problem1.ideaId], revealed: false, retry: false, at: past + 2000,
          },
        },
      ]);
    });
    await screen.findByText("Before try 3: what did you try?");
    expect(screen.queryByRole("group", { name: "What kind of miss was that?" })).toBeNull();
  });
});

...
export function ExplainStep({ step, quest, progress, householdId, profileId, onUpdate, onOwnPrimaryChange }: ExplainStepProps) {
  ...
  // meetsRequirements is the genuine teaching gate (a real, non-empty draft) -- the only reason
  // Save should ever show the disabled kraft fill; saveDisabled folds `saving` back in too, kept
  // for the onOwnPrimaryChange effect below exactly as before (a mid-save moment briefly stops
  // reporting an own primary, same as it always did), while the button's own `disabled` prop
  // below reads meetsRequirements alone so a save-in-flight shows the shared pending spinner
  // instead of the washed-out disabled fill (owner report: a disabled Save looks broken).
  const meetsRequirements = Boolean(text.trim());
  const saveDisabled = saving || !meetsRequirements;
  ...
  return (
    <section className="tr-step" aria-labelledby={`${step.id}-title`}>
      ...
      <div className="tr-step__actions">
        ...
        <Button
          variant={typedIsPrimary ? "primary" : "secondary"}
          onClick={handleSave}

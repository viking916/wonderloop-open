...
import { ExplanationOutcome, ExplanationPanel } from "./ExplanationPanel";
...
import { currentCycleFirstAttemptId, currentCycleProblemState, recordAttempt as recordAttemptDomain, viewProblem, type ProblemState } from "@/lib/domain/attempts";
...
export type ProblemPlayerProps = {
  ...
  reviewNote?: string;
  /**
   * "Ask" (12 September 2026): whether this problem's two authored hints are real, help-shaped
   * content rather than a placeholder standing in for "no hint here". Defaults to true --
   * true for every Think-track problem and every Ladder phase except stretch. LadderSession.tsx
   * passes false only for a stretch problem, whose hints are unhinted()'s synthetic "no hint on
   * a stretch problem" pair (schema.ts's hints tuple can never be genuinely empty, so a
   * placeholder is the only way stretch can say "no hint" at all) -- Ask would otherwise offer
   * to help past a difficulty the Ladder deliberately leaves unscaffolded. Retrieval, topic,
   * fluency and probe problems all draw the topic's own real bank hints unchanged and so stay
   * eligible, same as every Think problem.
   */
  hintsAuthored?: boolean;
};
 ...
 */
export function ProblemPlayer({ step, quest, progress, problemsProgress, progressLoaded, householdId, profileId, onUpdate, onFinished, onOwnPrimaryChange, reviewNote, hintsAuthored = true }: ProblemPlayerProps) {
  ...
  // Keeps each attempt's Firestore doc id alongside its data (watchAttemptsForQuest's own
  // shape), not just the AttemptDoc -- currentCycleFirstAttemptId below needs the id to know
  // which doc a miss-kind answer belongs to. Used to be flattened to AttemptDoc[] here, throwing
  // the id away; see currentCycleFirstAttemptId's own doc comment (lib/domain/attempts.ts) for
  // why that meant the miss-kind row's row could only ever come from a separate, racy,
  // in-session-only bit of state instead.
  ...
  // Every attempt on record for THIS problem, across every past cycle, id included -- NOT yet
  // scoped to the current cycle. currentCycleProblemState/currentCycleFirstAttemptId (lib/domain/
  // attempts.ts) do that scoping below, from this same filtered list, so "current cycle" means
  // the same thing live and after the fact (final review, finding C1) and the miss-kind row's id
  // (missAttemptId below) can never disagree with what triesUsed itself says.
  const attemptsForProblem = useMemo(() => attempts.filter((x) => x.attempt.problemId === problem.id), [attempts, problem.id]);

  const problemAttemptRecords = useMemo(
    () => attemptsForProblem.map((x) => ({ tryNumber: x.attempt.tryNumber, correct: x.attempt.correct, hintTier: x.attempt.hintTier, at: x.attempt.at })),
    [attemptsForProblem],
  );

  // "Ask" (12 September 2026): the child's own earlier wrong answers on this exact problem,
  // oldest first, briefed to the tutor (lib/ai/tutor.ts's TutorContext.attempts) so its first
  // reply already knows what has been tried rather than asking the child to repeat it.
  const priorAnswers = useMemo(
    () => attemptsForProblem.map((x) => x.attempt).sort((a, b) => a.at - b.at).map((a) => a.answer),
    [attemptsForProblem],
  );

  // Whether this family's AI key is configured (owner decision, 6 September 2026), the same
  // /api/ai/health check DebateRoom.tsx already makes -- checked once per household, not once
  // per problem, since it changes only when a parent edits the Parent view's AI card. undefined
  // while asking; a failed check counts as "on" so a network blip never hides Ask outright, the
  // same fail-open choice DebateRoom makes (a genuinely unconfigured key still gets caught, and
  // reported in words, the first time Ask actually tries to send).
  ...
  // check, and which problems have already been asked what kind of miss it was.
  ...
  // The current cycle's first attempt's own Firestore doc id, for the "What kind of miss was
  // that?" row below -- derived from attemptsForProblem (the same, listener-fed list that
  // drives view.triesUsed above), not from a separate in-session record of what handleSubmit's
  // own write last returned. See currentCycleFirstAttemptId's own doc comment (lib/domain/
  // attempts.ts) for why that used to both race view.triesUsed and not survive a reload.
  const missAttemptId = useMemo(
    () => currentCycleFirstAttemptId(attemptsForProblem.map((x) => ({ id: x.id, at: x.attempt.at })), cycleStart),
    [attemptsForProblem, cycleStart],
  );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // The returned id is not captured here: missAttemptId (above) derives the id the "What
      // kind of miss was that?" row needs straight from attemptsForProblem, the same
      // listener-fed list that already drives view.triesUsed, once this write's own attempt doc
      // round-trips back through watchAttemptsForQuest -- see currentCycleFirstAttemptId's own
      // doc comment (lib/domain/attempts.ts).
      await persistAttempt(householdId, profileId, {
  ...
  // The attempts listener (watchAttemptsForQuest) delivers the just-written attempt, and this
  // cooldown becomes known, well before handleSubmit's own later awaits (saveQuestProgress,
  // onUpdate, recomputeAndSaveSkills) let `busy` clear -- see handleSubmit's own comment on that
  // gap. Once inCooldown is true this is a timed lock, not work in flight: the coordinator's
  // review caught a spinner sitting on "Try again in 0:46" in the flat kraft fill, which lies
  // about what the app is doing (waiting out think time, not writing anything) and duplicates a
  // reason the label already states in words (styleguide's timed-lock rule). pending below is
  // gated on !inCooldown so the two states can never both show at once: the spinner belongs only
  // to the window before the outcome -- and therefore the cooldown -- is known.
  const inCooldown = view.cooldownEndsAt !== undefined && now < view.cooldownEndsAt;
  const buttonLabel = inCooldown ? `Try again in ${formatMMSS(view.cooldownEndsAt! - now)}` : "Check";
    ...
    return (
      <section className="tr-step tr-problem tr-clock-offer" aria-labelledby={`${step.id}-clock-title`}>
        ...
        <div className="tr-step__actions">
          <Button variant="primary" onClick={() => onUpdate((prev) => startClock(prev, step.id, Date.now()))}>
            Start the clock
          </Button>
          <Button variant="quiet" onClick={() => onUpdate((prev) => declineClock(prev, step.id))}>
  ...
  return (
    <section className="tr-step tr-problem" aria-labelledby={`${problem.id}-title`}>
      {/* Laptop-width polish (15 September 2026, owner: "why are we showing tablet style
          constrained UI on laptop view?"): above 1280px this splits into a middle column (the
          question and everything that answers it) and a right column (what used to just stack
          underneath -- the working space, the hint, Ask, the think-time timer, and once answered,
          the explanation and idea card), with a footer spanning both for the last-problem note
          and the Skip/Try again/Next problem/Next step row, so it is never read ahead of the
          explanation it follows. Below 1280px .tr-problem__layout is a single column in plain DOM
          order (main, then side, then footer) -- the same stacked shape this screen already had. */}
      <div className="tr-problem__layout">
        <div className="tr-problem__main">
          <div className="tr-pnum">
            {problems.length > 1 ? (
            ...
            <>
              {view.outcome === "unanswered" ? (
                <>
                  ...
                  {view.triesUsed === 1 && missAttemptId && !isRetryCycleFor(problem.id) ? (
                      ...
                      {(["misread", "did-not-know", "slipped"] as MissKind[]).map((k) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            void setAttemptMissKind(householdId, profileId, missAttemptId, k).catch(() => undefined);
                          }}
                        >
                          {MISS_LABEL[k]}
                        </button>
                  ...
                  {/* Laptop-width polish: "Skip for now" is the only nav control that ever shows
                      while a problem is unanswered, so it stays in the middle column right under
                      the answer, rather than waiting with "Try it again"/"Next problem"/"Next
                      step" for the explanation (see below) -- those only ever appear once he has
                      actually answered. */}
                  <div className="tr-step__actions tr-step__actions--nav">
              ...
              {view.outcome === "exhausted" && !view.explanationOpen ? (
                // "Try it again" already reads (view.outcome !== "unanswered") during this
                // cooldown, before the explanation itself is even open -- the cooldown timer
                // itself is in .tr-problem__side below, but the nav control stays in the middle
                // column with every other nav row on this screen.
                <div className="tr-step__actions tr-step__actions--nav">
                  <Button variant="quiet" onClick={handleRetry} pending={busy} pendingLabel="Starting again">
                    Try it again
                  </Button>
                </div>
              ) : null}
              ...
              {view.explanationOpen ? (
                <>
                  {/* Laptop-width polish, round 2 (15 September 2026): the owner's screenshot
                      caught the whole explanation squeezed into the right column's 340-380px
                      while the middle column sat mostly empty under the question. The result
                      banner and the nav row now sit right under the question, at the middle
                      column's own width, with the explanation's body (steps, figure, use-again)
                      and the idea card following in the same column -- the right column is not
                      part of this state at all (see .tr-problem__side below), so working space
                      moves in here too rather than being the only thing left stranded there. */}
                  <ExplanationOutcome outcome={view.outcome} />
                  {index >= problems.length - 1 ? <p className="tr-step__done">That was the last problem in this set.</p> : null}
                  {index >= problems.length - 1 && clock && minutesToFinish(clock) !== undefined ? (
                  ...
                  <div className="tr-step__actions tr-step__actions--nav">
                    ...
                    {index >= problems.length - 1 && onFinished ? (
        ...
        <div className="tr-problem__side">
          {/* Task 43: one quiet button, per problem id, that opens the working space -- present on
              every problem while the explanation is not open, since he might want to work
              something out before ever pressing Check. Once answered, working space moves into
              the middle column above (see the explanationOpen branch) rather than being the only
              thing left in this column. Keyed by problem.id: switching problems (or reaching the
              same problem again on "Try it again", where the key stays the same on purpose -- a
              working belongs to the problem id, not one attempt cycle) mounts/re-hydrates the
              right one. */}
          {!view.explanationOpen ? (
            <>
              <WorkingSpace
              ...
              {/* Task 43 (spec: "the return is the point"): on a mistake-box return, whatever he
                  wrote or drew here before this sitting shows again, read-only, right alongside
                  the variant -- not tucked inside the working-space panel he would have to think
                  to open. pastWorking is frozen at the first load for this problem id (see its
                  own comment above), so reopening the panel above and adding to it during THIS
                  sitting never changes what this block shows. */}
              {quest.id === REVIEW_QUEST_ID && hasWorkingContent(pastWorking) ? (
                <PastWorking heading="Your working from last time" content={pastWorking!} />
              ) : null}
            </>
          ...
          {hydrated && view.outcome === "unanswered" && view.triesUsed > 0 ? (
          ...
          {hydrated && view.outcome === "exhausted" && !view.explanationOpen ? (
            <div className="tr-cool">
              <CooldownTimer

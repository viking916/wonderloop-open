...
export function ProblemPlayer({ step, quest, progress, problemsProgress, progressLoaded, householdId, profileId, onUpdate, onFinished, onOwnPrimaryChange, reviewNote, hintsAuthored = true }: ProblemPlayerProps) {
  ...
  const priorAnswers = useMemo(
    () => attempts.filter((a) => a.problemId === problem.id).sort((a, b) => a.at - b.at).map((a) => a.answer),
    [attempts, problem.id],
  );
    ...
    <section className="tr-step tr-problem" aria-labelledby={`${problem.id}-title`}>
      ...
      {!hydrated ? (
        ...
        <>
          ...
          {/* Polish pass (Package B): "Try it again | That was the last problem in this set. |
              Next step" used to sit on one flex-wrapped row, putting a full sentence between two
              controls on three different baselines (styleguide 2: "the control that unlocks the
              next action sits above that action"). The message now stands on its own line, above
              a single 48px control row -- quiet on the left, primary at the row's right edge. */}
          {view.explanationOpen && index >= problems.length - 1 ? (
            <p className="tr-step__done">That was the last problem in this set.</p>
          ) : null}
          {view.explanationOpen && index >= problems.length - 1 && clock && minutesToFinish(clock) !== undefined ? (
          ...
          <div className="tr-step__actions tr-step__actions--nav">
            ...
            {view.explanationOpen && index >= problems.length - 1 && onFinished ? (

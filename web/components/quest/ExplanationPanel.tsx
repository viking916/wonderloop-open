...
// Polish pass (Package B item 3, 15 September 2026): every content/ useAgain string is authored
// as the lowercase tail of "Use it again ___" ("when you need to check whether..."), which read
// fine inline but now stands alone as its own body line under a separate "Use it again" eyebrow
// (see .tr-explanation__use-again below), so a lowercase "when..." looked like a sentence cut off
// mid-thought. This capitalises the body's own first letter for display only -- content/ is never
// edited. A future author writing the redundant "Use it again" prefix directly into the field is
// stripped first, so the eyebrow and body can never say the same words twice; content that does
// not carry that prefix (every quest today) keeps its own wording exactly, just with its first
// letter capitalised.
const USE_AGAIN_PREFIX = /^use it again\b[,:]?\s*/i;

function capitalizeUseAgain(text: string): string {
  const body = text.replace(USE_AGAIN_PREFIX, "");
  if (!body) return body;
...
/**
 * Laptop-width polish, round 2 (15 September 2026): the tr-outcome badge ("That's right.") used
 * to open ExplanationPanel itself, so the whole explanation -- banner, steps, figure, use-again --
 * had to move together into the right column above 1280px, which squeezed a long explanation into
 * a 340-380px strip while the middle column sat empty under the question. Split out so
 * ProblemPlayer can put the banner and the nav row (Try it again / Next problem / Next step)
 * directly under the question, in the middle column, with the explanation's own body following
 * at the middle column's full width -- see ProblemPlayer.tsx's own comment on .tr-problem__main.
 */
export function ExplanationOutcome({ outcome }: { outcome: ProblemView["outcome"] }) {
 ...
 * explanationOpen is true. The note underneath names the actual reason it opened without
 * credit -- "revealed" (a not-auto-graded text/proof problem, which always opens on its first
 * submission) reads differently from "exhausted" (three wrong tries plus the think-time floor) --
 * so a family reading it never sees a vague "no credit" line that does not match what actually
 * happened. A rubric problem's mustMention phrases (spec 7.4: "the rubric phrases stay hidden
 * until the explanation opens") only ever render here, never before. The outcome banner itself
 * (Task 10 fix, Critical finding 2) is now ExplanationOutcome, above, rendered separately so it
 * can sit right under the question while this body sits further down the same column.
 */
export function ExplanationPanel({ problem, outcome, retry = false }: ExplanationPanelProps) {
  return (
    <div className="tr-explanation">
      ...
      <div className="tr-explanation__use-again">
        <span className="tr-eyebrow">Use it again</span>
        <p>{capitalizeUseAgain(problem.useAgain)}</p>
      </div>

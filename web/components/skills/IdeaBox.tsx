...
import { groupIdeaEvidenceLines } from "@/lib/domain/ideaEvidence";
            ...
            return (
              <article key={met.ideaId} className="tr-idea-list__item">
                ...
                <ul className="tr-idea-list__problems">
                  {groupIdeaEvidenceLines(met.problemIds.map(describeProblem)).map((line) => (
                    <li key={line.text} className="tr-meta">
                      {line.text}
                    </li>
                  ))}
                </ul>

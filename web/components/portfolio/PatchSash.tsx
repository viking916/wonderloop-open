...
export function PatchSash({ progress }: PatchSashProps) {
  ...
  return (
    <aside className="tr-sash">
            ...
            {earned.map((badge: Badge) => {
              ...
              return (
                <Patch
                  ...
                  caption={badge.earnedAt !== undefined ? formatDateShort(badge.earnedAt) : undefined}
                />
              );
            })}
          </div>
      ...
      {/* Round 2: one dashed group panel (the placeholder, styleguide 1) holding plain rows --
          icon, name, a muted "for..." line beneath -- instead of a dashed chip per badge with its
          own wrapped sentence. */}
      {locked.length > 0 ? (
        <div className="tr-sash__section">
          <p className="tr-sash__label">Still out there</p>
          <div className="tr-patches-group">
            {locked.map((badge) => {
              const Icon = ICONS[badge.id];
              return (
                <div className="tr-patches-group__row" key={badge.id}>
                  <span className="tr-patches-group__icon" aria-hidden="true">
                    {Icon ? <Icon /> : null}
                  </span>
                  <div className="tr-patches-group__text">
                    <p className="tr-patches-group__name">{badge.name}</p>
                    <p className="tr-patches-group__why">{invitationFor(badge.id)}</p>
                  </div>
                </div>
              );
            })}

...
// like something is broken") applied to Sprout, under styleguide section 5's own absolute rules:
// nothing readable required, nothing failable, no dead ends. A tap on a hill thing calls
// onOpenActivity, which is a plain router.push -- invisible to the shared route progress bar
// (RouteProgressBar.tsx only watches anchor clicks) and returning no Promise the shared Button
// component could auto-track. So this hill draws its own two-part feedback instead of reusing
// Button: an immediate pressed state on the tapped pill itself (pointerdown/up, not click, so it
// shows before the tap even completes), and -- only if the navigation is still not done by
// PENDING_SHOW_DELAY_MS later (a cold dev-server compile of the activity route, mainly) -- a
// quiet, wordless pulse on the same pill (lib/pendingTiming.ts's shared anti-flicker timing, the
// same numbers Button.tsx and RouteProgressBar.tsx use, so a fast open never flickers one). Never
// a spinner with a text label (a 3-year-old cannot read "Working"): see .sp-thing--opening in
// globals.css, a plain glow, no glyph.
//
// OPENING_SAFETY_MS: a real client-side navigation always lands (or this component unmounts
// entirely, which throws the state away for free) -- but should it ever not, taps must never
// stay dead forever (styleguide 5: "no dead ends"). This clears the pending state and lets the
// same or a different thing be tapped again.
const OPENING_SAFETY_MS = 8000;
...
export function Hill({ activities, doneIds, stars, onOpenActivity, parentGate }: HillProps) {
  ...
  // Immediate press feedback (pointerdown/up, not click): see the block comment above the
  // OPENING_SAFETY_MS constant. Cleared on pointerup/leave/cancel the same way ParentGate's own
  // hold gesture resets, so a finger that lands and drags off the pill never leaves it stuck
  // looking pressed.
  const [pressedId, setPressedId] = useState<string | undefined>(undefined);
  // Which thing is actually navigating; blocks a second tap (its own or another thing's) from
  // starting a second, competing navigation while this one is in flight -- the same "ignore, do
  // not disable" rule Button.tsx's own pending click-guard uses.
  const [openingId, setOpeningId] = useState<string | undefined>(undefined);
  const showOpening = useDelayedPending(openingId !== undefined);
  ...
  function handleTap(activityId: string) {
    if (openingId !== undefined) return;
    setOpeningId(activityId);
    if (safetyTimer.current) clearTimeout(safetyTimer.current);
    safetyTimer.current = setTimeout(() => setOpeningId(undefined), OPENING_SAFETY_MS);
  ...
  return (
    <div className="sp-world">
      ...
      {activities.map((activity, i) => {
        ...
        const pressed = pressedId === activity.id;
        const opening = showOpening && openingId === activity.id;
        return (
          <button
            key={activity.id}
            type="button"
            className={`sp-thing${done ? " sp-thing--done" : ""}${pressed ? " sp-thing--pressed" : ""}${opening ? " sp-thing--opening" : ""}`}
            style={{ left: pos.left, top: pos.top, bottom: pos.bottom }}
            aria-label={`${activity.title}${done ? ". Done." : ""}`}
            aria-busy={opening ? "true" : undefined}
            onPointerDown={() => setPressedId(activity.id)}
            onPointerUp={() => setPressedId((p) => (p === activity.id ? undefined : p))}
            onPointerLeave={() => setPressedId((p) => (p === activity.id ? undefined : p))}
            onPointerCancel={() => setPressedId((p) => (p === activity.id ? undefined : p))}

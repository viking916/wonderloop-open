...
import type { MonsterCardData, MonsterStatus } from "../../lib/domain/monster";

export type { MonsterCardData, MonsterStatus };
...
export interface SideStripProps {
  ...
  game?: GameCardData;
  /** Returns the underlying write's own Promise so the "We played it" Button below can
   * auto-track it as pending (see Button.tsx's Promise-tracking doc comment) -- owner report:
   * a button that does nothing visible for a moment reads as broken. */
  onGamePlayed?: (gameId: string) => Promise<void>;
  /** Set whenever the displayed week's sprint (weeks 1-4, 5-8, 9-12) has an authored monster (A1,
   * 2026-09-24): visible for that whole sprint, not just its own week, and never gates the Think
   * quest's completion (lib/domain/completion.ts). An unsolved monster from the sprint just
   * before is carried over instead, marked "still-open" (lib/domain/monster.ts's
   * findSprintMonster has the full carry-over rule). */
    ...
    <div className={stripClasses}>
      {monster ? (
        <Card tone="forest" className="tr-note tr-note--monster">
          ...
          <h3>{monster.title}</h3>
          <p>{monster.prompt}</p>
          <p className="tr-note--monster-status">
            {monster.status === "solved"
              ? "Solved."
              : monster.status === "still-open"
                ? "Still open from an earlier sprint, until it is solved or the next one arrives."
                : "Open all month. No rush; the worked solution opens as soon as you answer."}
          </p>
          <Button variant="secondary" className="tr-note--monster-open" href={monster.href}>
            Open the monster
          </Button>
        </Card>
      ) : null}

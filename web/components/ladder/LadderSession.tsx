...
import {
  ...
  applyProbe, applyRetrievalResult, breakActive, emptyLadderState, emptyTopicState, fluencyPassed, guessingRun, ladderWeek, markMastered, masteryBarMet,
  nextTopic, pickFluencyProblems, pickProbeProblems, pickRetrievalProblems, pickStretch, pickTopicProblems, placementCandidates,
  recordTopicResult, retrievalDue, retrievalEntriesFor, stageSummary,
...
/**
 * Laptop-width polish, round 2 (15 September 2026): the surface card around the Ladder's own
 * content used to be fixed in app/explorer/ladder/page.tsx, always at .tr-page's full width.
 * That is right for the plan and round screens, but the break screen is one message and two
 * buttons, and a 1360px-wide card around one short line read as empty (the owner's screenshot).
 * LadderSession now wraps its own phase's content in this card, so a phase can ask for a
 * narrower, centred one (see `ld-break-card` below) instead of always inheriting the page's own
 * width.
 */
function withCard(content: ReactNode, extraClass?: string) {
  return (
    <Card tone="surface" shadow className={extraClass ? `!p-0 ${extraClass}` : "!p-0"}>
      <div className="tr-quest__main ld-main">{content}</div>
    </Card>
  );
}
...
export function LadderSession({ householdId, profileId }: { householdId: string; profileId: string }) {
  ...
  async function startOrResumeSession() {
    ...
    if (openEntry) {
      ...
      // "Skip the break" (15 September 2026): a break is enforced across a reload too, via
      // LadderState.openSession.breakUntil (lib/domain/ladder.ts's breakActive, pure so this and
      // the running countdown below ask the same question the same way) -- carried forward here
      // only while it is still genuinely active, never written as an explicit `undefined`.
      const persistedBreakUntil = base.openSession?.breakUntil;
      const resumingBreak = breakActive(persistedBreakUntil, now);
      const next: LadderState = {
        ...base,
        openSession: { number, day: DAY_NAMES[day], round, startedAt, ...(resumingBreak ? { breakUntil: persistedBreakUntil } : {}) },
      };
      ...
      if (resumingBreak) {
        setBreakUntil(persistedBreakUntil);
        setNote(undefined);
        setPhase("break");
      } else {
  ...
  function endRound(fluencyNote?: string) {
    ...
    // "Skip the break": breakUntil is written to LadderState.openSession, not just held in local
    // state, so the break is still enforced (or already skipped) after a reload -- see
    // breakActive in lib/domain/ladder.ts and startOrResumeSession's own resume-into-break check.
    const until = Date.now() + BREAK_MINUTES * 60_000;
    setBreakUntil(until);
    setSessionBoth({ ...session, round: nextRound });
    setPhase("break");
    void saveLadderState(householdId, profileId, {
      openSession: { number: session.number, day: DAY_NAMES[session.day], round: nextRound, startedAt: session.startedAt, breakUntil: until },
    }).catch(() => undefined);
  }
  ...
  /** "Skip the break" (owner ask, 15 September 2026: the Ladder's five-minute breaks between
   * rounds are optional, not a lock). Loses nothing, so this needs no confirm -- it only collapses
   * the break's own end time to right now, both locally (the countdown reads 0 on the very next
   * tick, same as a break that finished on its own) and in Firestore (breakActive, above, then
   * reports the break as over for a reload too), rather than deleting the field outright. The
   * "Next round" button wakes up the instant this runs; the round itself never auto-starts, so a
   * child who skips still presses "Next round" to actually move on. */
  function skipBreak() {
    if (!session) return;
    const until = Date.now();
    setBreakUntil(until);
    void saveLadderState(householdId, profileId, {
      openSession: { number: session.number, day: DAY_NAMES[session.day], round: session.round, startedAt: session.startedAt, breakUntil: until },
    }).catch(() => undefined);
  }
  ...
  if (authored.length === 0) {
    return withCard(<EmptyState title="The Ladder opens soon" description="The first topics are being written. Check back in November." action={<Button variant="primary" href="/explorer">Back to This week</Button>} />);
  }
  if (!session) {
    ...
    return withCard(
      <div className="ld-home">
        ...
        {/* Laptop-width polish, round 2 (15 September 2026): the page title and its meta line
            used to live inside the right-hand topic card, so the page itself had no heading where
            the eye starts and that card read as a thin, near-empty box beside a much taller plan.
            The title and meta now span the top of the whole card, the way every other screen's
            .tr-page-title does; the two columns below it are the plan (left) and a useful "where
            you are" panel (right: the six-stage summary, the current stage named in words, and
            the current topic's own one-line idea when there is one) rather than one column with
            almost nothing in it. */}
        {!state ? (
          <>
            <h2 className="tr-page-title">Start the Ladder</h2>
            <p className="ld-home__facts tr-meta">The first session is a placement: short probes to find where you are. After that, every session is rounds of 25 minutes with a break between.</p>
          </>
        ) : (
          <>
        ...
        <div className="ld-home__layout">
          <div className="ld-home__main">
            {/* The week on the Ladder (13 September 2026): a plan of three sessions, finished in
                order, that waits instead of expiring. Named "session 1, 2 and 3 of the week" on
                this child-facing screen, per the owner's decision, never by weekday. */}
            ...
            <Button variant="primary" onClick={startOrResumeSession}>{primaryLabel}</Button>
            <Button variant="quiet" href="/explorer" className="tr-quest__park">Back to This week</Button>
          </div>
          <div className="ld-home__side">
            <p className="tr-eyebrow">Where you are</p>
            <ul className="ld-home__stage-list">
              {stageSummary(graph.filter((t) => topicsById[t.id]), topicStates, state?.currentTopic).map((s) => (
                <li key={s.stage} className={`ld-home__stage-row ld-home__stage-row--${s.status}`}>
                  <span className="ld-home__stage-title">{s.title}</span>
                  <span className="tr-meta">{s.status === "current" ? "You are here" : `${s.mastered} of ${s.total}`}</span>
                </li>
              ))}
            </ul>
            {currentTopicMeta ? (
              <div className="ld-home__current">
                <p className="ld-home__current-title">{currentTopicMeta.title}</p>
                <p className="tr-meta">{currentTopicMeta.summary}</p>
              </div>
            ) : null}
          </div>
        </div>
  ...
  if (phase === "break") {
    ...
    return withCard(
      <div className="ld-break">
        ...
        {/* Round 2 fix (15 September 2026): "Ready" used to reuse the countdown's own mono,
            timer-sized heading, and the sentence still told him to wait or skip even once the
            break was already over (naturally, or via "Skip the break"). The clock (mono, big) and
            the "over" heading (the ordinary heading font, .tr-page-title's own size) are now two
            different elements, so the over state reads as a real heading, not a stuck timer. */}
        {over ? (
          <h2 className="tr-page-title">Break over</h2>
        ) : (
          <h2 className="ld-break__clock">{`${Math.floor(left / 60)}:${String(left % 60).padStart(2, "0")}`}</h2>
        )}
        <p>
          {over
            ? "Start the next round when you are ready."
            : "This break is optional. Wait for it to finish, or skip it and keep going. Stand up, water, look out of a window."}
        </p>
        <div className="ld-actions">
          ...
          {/* Quiet tier, not primary: the disabled "Next round" above stays the group's one
              primary the whole time, per styleguide 1's "one primary per group" -- this only
              wakes it up early. No confirm needed; skipping loses nothing. */}
          {left > 0 ? (
            <Button variant="quiet" onClick={skipBreak}>
              Skip the break
            </Button>
          ) : null}
        ...
        {currentTopic.ways?.length ? (
          <WaysChooser
            ...
            onPick={(wayId) => {
              ...
              return saveTopicState(householdId, profileId, currentTopic.id, { ...ts, favouriteWay: wayId });
            }}
          />
        ) : null}
 ...
 */
function WaysChooser({ ways, favourite, onPick }: { ways: NonNullable<LadderTopic["ways"]>; favourite?: string; onPick: (wayId: string) => Promise<unknown> | void }) {
    ...
    <Card tone="surface" className="ld-ways">
      ...
      {ways.filter((w) => w.id === open).map((w) => (
        <div key={w.id} className="ld-way" role="tabpanel">
          ...
          <Button
            variant={favourite === w.id ? "secondary" : "primary"}
            onClick={() => onPick(w.id)}
            disabled={favourite === w.id}
            pendingLabel="Saving your way"
          >

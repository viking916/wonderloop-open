...
export function AiKeyCard({ householdId }: { householdId: string }) {
  ...
  // Which of the two actions is actually running, not just "is anything running" -- Save and
  // Remove can both be on screen at once (the form stays visible even once a key is configured,
  // to offer "Replace the key"), so each button needs to know whether it is the one waiting on
  // the real network call to Anthropic (owner note: "seconds long") versus merely blocked because
  // its sibling is mid-flight.
  const [busyAction, setBusyAction] = useState<"save" | "remove" | undefined>(undefined);
      ...
      <div className="pr-aikey__form">
        ...
        <div className="tr-step__actions">
          <Button
            ...
            pendingLabel="Checking your key with Anthropic"
            disabled={busyAction === "remove" || !draft.trim()}
          >
            Check and save
          </Button>
        </div>

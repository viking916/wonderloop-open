...
export function PinSettings({ householdId, household }: PinSettingsProps) {
  ...
  // Returns its own Promise for Button.tsx's automatic tracking to watch -- this is the only
  // control this state ever drove, so there is no other element that still needs to see "busy".
      ...
      {editing ? (
        ...
        <>
          ...
          <div className="pr-pin__row">
            ...
            {hasPin ? (
              <Button variant="secondary" onClick={() => turnOff()} pendingLabel="Turning off the PIN">
                Turn off PIN
              </Button>
            ) : null}
          </div>

...
import { TRACK_ORDER, type Track } from "@/lib/content/schema";
...
export function ParentActivities({ profile, householdId }: ParentActivitiesProps) {
  ...
  const skillOptions = useMemo(() => {
    // This is a free-form "log a skill against anything, including chess or piano practised
    // outside the app" picker, not the weekly quest flow -- tracksForProfile (lib/domain/
    // tracks.ts) never returns "play" any more (Play stopped being a quest on 23 September
    // 2026), but a parent must still be able to log a Play skill note for a profile with the
    // track on, so this reads TRACK_ORDER directly instead.
    const tracks: Track[] = profile.kind === "sprout" ? [] : TRACK_ORDER.filter((t) => t !== "play" || profile.playTrack === true);
    ...
    <Card tone="surface" className="pr-activities" aria-labelledby={`activities-${profile.id}`}>
      ...
      <form className="pr-activities__form" onSubmit={(e) => void submit(e)}>
        ...
        <Button
          ...
          type="submit"
          pending={saving}
          pendingLabel="Saving the activity"

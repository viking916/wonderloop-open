...
function BucketRows({
  ...
  /** Returns its own Promise (never voided by the caller) so each row's Button can track its own
   * write to Firestore automatically -- no shared busy state needed, since every row's Button
   * keeps its own pending state independently. */
  onToggle: (key: string, bought: boolean) => Promise<void>;
}) {
  return (
            ...
            <div className="pr-shopping__row-actions">
              ...
              {bought ? (
                ...
                <>
                  ...
                  <Button variant="quiet" onClick={() => onToggle(item.key, !bought)} pendingLabel="Undoing">
                    Undo
                  </Button>
                </>
              ) : (
                <Button variant="secondary" onClick={() => onToggle(item.key, !bought)} pendingLabel="Saving">
                  Mark as bought
                </Button>
              )}
            </div>
...
export function SeasonShoppingList({ householdId, household, seasonId, currentWeek, seasonIds }: SeasonShoppingListProps) {
  ...
  const toggle = useCallback(
    (key: string, bought: boolean) => setMaterialBought(householdId, key, bought),
    [householdId],
  );

    ...
    <li className="pr-log__row">
      ...
      <div className="pr-log__comment">
        ...
        <div className="tr-answer__row">
          ...
          />
          <Button variant="secondary" onClick={() => save()} pending={saving} pendingLabel="Saving your comment">
            {saved ? "Saved" : "Save"}
          </Button>
        </div>
  ...
  return (
    <Card tone="surface" shadow className="pr-loglist">
              ...
              <li key={item.problemId} className="pr-mistakebox__row pr-mistakebox__row--wrap">
                ...
                <div className="pr-mistakebox__actions">
                  ...
                  {item.approved ? (
                    ...
                    // Returns approve's own Promise (never voided) so Button.tsx's automatic
                    // tracking can show this waiting on Firestore -- there was no feedback here
                    // at all before, and this write had no busy state of its own to wire up.
                    <Button variant="secondary" onClick={() => approve(item.questId, item.problemId)}>
                      Thumbs up
                    </Button>
                  )}
                  <ConfirmResetButton
      ...
      {/* Task 43: read-only, the same way the mistake-box return is -- nothing here approves,
          ...
          "here is what actually got done" sections this card already shows. */}
      <section aria-labelledby={`workings-${profile.id}`}>
        <p className="tr-eyebrow" id={`workings-${profile.id}`}>
          Working spaces
        </p>
        {workingItems.length === 0 ? (
          <EmptyState title="Nothing saved yet" description="Notes or a drawing from the working space appear here once one is saved." />
        ) : (
          <ul className="pr-mistakebox__list">

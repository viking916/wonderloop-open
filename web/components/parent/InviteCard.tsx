 ...
 *
 * Debugged 15 September 2026: this used to be tone="kraft", a kraft panel sitting directly on the
 * Settings tab's own kraft ground, while every other settings card is tone="surface" -- white on
 * kraft, the surface every other card in the app uses too. Kraft-on-kraft here was not a
 * deliberate destructive-tier look (only Button variant="destructive" carries that meaning, per
 * ResetControls' own doc comment); it just made this one card look inconsistent for no reason.
 */
export function InviteCard({ householdName, inviteCode }: InviteCardProps) {
  ...
  return (
    <Card tone="surface" className="pr-invite" aria-labelledby="invite-card-heading">
      ...
      <div className="pr-invite__code-row">
        ...
        {/* copyCode's own Promise is returned (not voided) so Button.tsx's automatic tracking
            can watch it -- clipboard access can hang on a permission prompt, and this had no
            feedback of any kind before. */}
        <Button variant="secondary" onClick={copyCode} pendingLabel="Copying the code">

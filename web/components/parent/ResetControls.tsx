        ...
        {done ? (
          ...
          <>
            ...
            <div className="pr-confirm__row">
              ...
              {/* Task's own resets note: these delete the most work and must not look dead or be
                  double-pressable. `pending` (never `disabled`) both keeps the destructive tier's
                  double border and warning icon intact -- a real `disabled` would flatten it to
                  the same kraft fill every other tier gets, exactly the look the owner called
                  broken -- and blocks a second click by ignoring it, the same rule Button.tsx
                  applies everywhere else. */}
              <Button variant="destructive" onClick={() => confirm()} pending={pending} pendingLabel="Resetting">
                Yes, reset
              </Button>
            </div>
            {error ? (
 ...
 * per-item control.
 *
 * Debugged 15 September 2026: the card itself used to be tone="kraft" (a kraft panel on the
 * Settings tab's own kraft ground) while every sibling settings card is tone="surface" -- white,
 * the same as everywhere else in the app. The destructive tier is carried entirely by
 * ConfirmResetButton's own Button variant="destructive" and its dialog, never by the card's
 * background, so making the surface consistent loses none of the "this is different" signal.
 */
export function ResetControls({ profile, householdId, currentWeek, children }: ResetControlsProps) {
  ...
  return (
    <Card tone="surface" className="pr-resets" aria-labelledby={`reset-controls-${profile.id}`}>

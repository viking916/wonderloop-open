...
export function KidCard({ profile, currentWeek, doneLabel, leftLabel, minutesSpentThisWeek }: KidCardProps) {
  const kindLabel = profile.kind === "explorer" ? "Explorer" : "Sprout";
  // Polish pass, 15 September 2026: a profile named "Explorer" or "Sprout" used to see the same
  // word twice, once as the name and once right under it in the eyebrow. Hide the kind word when
  // it only repeats the name, case-insensitively -- the same rule ProfilePicker's tiles apply.
  const showKind = kindLabel.toLowerCase() !== profile.name.trim().toLowerCase();
  return (
      ...
      <div className="pr-kid-card__head">
        ...
        <div>
          ...
          <p className="tr-eyebrow">
            {showKind ? `${kindLabel} · ` : ""}Week {currentWeek} of 12
          </p>
        </div>
      </div>

...
import {
  applyProbe, applyRetrievalResult, asVariant, bandFor, breakActive, emptyTopicState, fluencyPassed, guessingRun, ladderWeek, masteryBarMet,
  nextTopic, pickFluencyProblems, pickProbeProblems, pickRetrievalProblems, pickStretch, pickTopicProblems, placementCandidates,
  recordTopicResult, retrievalDue, retrievalEntriesFor, roundPlan, shuffle, stageSummary, RETRIEVAL_CAP, SESSIONS_PER_WEEK, WEEK_SESSION_ROUNDS,
} from "./ladder";
...
describe("skip the break", () => {
  it("is active before its own end time and over once that time is reached", () => {
    const now = 1_000_000;
    expect(breakActive(now + 1, now)).toBe(true);
    expect(breakActive(now, now)).toBe(false);
    expect(breakActive(now - 1, now)).toBe(false);
  });

  it("reports no break at all when nothing is open", () => {
    expect(breakActive(undefined, Date.now())).toBe(false);
  });

  it("skipping collapses the break's own end time to now, which reads as over immediately and stays over on a later check (a reload)", () => {
    const breakStartedAt = 1_000_000;
    const scheduledEnd = breakStartedAt + 5 * 60_000;
    expect(breakActive(scheduledEnd, breakStartedAt + 1_000)).toBe(true);
    // "Skip the break" collapses breakUntil to the moment it was pressed, never deletes it.
    const skippedAt = breakStartedAt + 1_000;
    expect(breakActive(skippedAt, skippedAt)).toBe(false);
    // A reload (or any later check) never brings the lock back once skipped.
    expect(breakActive(skippedAt, skippedAt + 60_000)).toBe(false);
  });
});

...
describe("A1: a monster step no longer blocks a Think quest (2026-09-24 year refinement)", () => {
  // s1-w11-think carries a real monster (lane "monster", step s1-w11-think-05), ahead of its two
  // trailing explain steps -- exactly the ordering that makes resumePosition's "skip once
  // everything else is done" rule worth a real test, not just isDone's.
  const monsterQuest = quest("s1-w11-think");
  const monsterStep = monsterQuest.steps.find((s) => s.kind === "problem-set" && s.lane === "monster");
  if (!monsterStep) throw new Error("expected s1-w11-think to carry a monster step");
  const monsterIndex = monsterQuest.steps.indexOf(monsterStep);

  function nonMonsterProblemIds(q: Quest): string[] {
    const ids: string[] = [];
    for (const step of q.steps) {
      if (step.kind === "warmup") ids.push(...step.problems.map((p) => p.id));
      if (step.kind === "problem-set" && step.lane !== "monster") ids.push(...step.problems.map((p) => p.id));
      if (step.kind === "puzzle-of-week") ids.push(step.problem.id);
    }
    return ids;
  }

  function progressWithEverythingButMonsterDone(): QuestProgress {
    const explainSteps = monsterQuest.steps.filter((s) => s.kind === "explain");
    const otherTickable = monsterQuest.steps.filter((s) => ["instruction", "science", "typing", "task"].includes(s.kind));
    return {
      ...emptyQuestProgress(),
      ticks: otherTickable.map((s) => ({ stepId: s.id, done: true })),
      problemOutcomes: outcomesFor(nonMonsterProblemIds(monsterQuest), "correct"),
      explains: Object.fromEntries(explainSteps.map((s) => [s.id, { text: "I used the idea." }])),
    };
  }

  test("isStepComplete on the monster step itself is unaffected: still false while unanswered", () => {
    expect(isStepComplete(monsterStep, emptyQuestProgress())).toBe(false);
  });

  test("questStatus is done once every step but the monster is complete", () => {
    const progress = progressWithEverythingButMonsterDone();
    expect(isStepComplete(monsterStep, progress)).toBe(false); // the monster really is still unanswered
    expect(questStatus(monsterQuest, progress)).toBe("done");
  });

  test("resumePosition skips the monster once everything else is done, landing past the end", () => {
    const progress = progressWithEverythingButMonsterDone();
    expect(resumePosition(monsterQuest, progress)).toEqual({ stepIndex: monsterQuest.steps.length });
  });

  test("resumePosition still stops at the monster while an earlier step is genuinely unanswered", () => {
    const before = monsterQuest.steps.slice(0, monsterIndex);
    const priorProblemIds = before.flatMap((s) =>
      s.kind === "warmup" || (s.kind === "problem-set" && s.lane !== "monster")
        ? s.problems.map((p) => p.id)
        : s.kind === "puzzle-of-week"
          ? [s.problem.id]
          : [],
    );
    const priorTicks = before
      .filter((s) => ["instruction", "science", "typing", "task"].includes(s.kind))
      .map((s) => ({ stepId: s.id, done: true }));
    const progress: QuestProgress = { ...emptyQuestProgress(), ticks: priorTicks, problemOutcomes: outcomesFor(priorProblemIds, "correct") };
    // The monster is itself a problem-set step, so a stop there also reports problemIndex 0 (its
    // one unanswered problem), the same shape any other unanswered problem-set step would report.
    expect(resumePosition(monsterQuest, progress)).toEqual({ stepIndex: monsterIndex, problemIndex: 0 });
  });

  test("weekSummary's minutesLeft is 0 once everything but the monster is done", () => {
    const progress = progressWithEverythingButMonsterDone();
    const summary = weekSummary([monsterQuest], { [monsterQuest.id]: progress });
    expect(summary[0].status).toBe("done");
    expect(summary[0].minutesLeft).toBe(0);
  });

  test("weekSummary's minutesLeft never counts the monster while other steps are still open", () => {
    // Nothing done at all: minutesLeft should be the full quest length, exactly like any other
    // untouched quest, whether or not one of its steps happens to be a monster.
    const summary = weekSummary([monsterQuest], {});
    expect(summary[0].minutesLeft).toBe(monsterQuest.minutes);
  });
});
...
describe("weekSummary", () => {
  ...
  test("a quest that spans two calendar weeks (sittings: 2) still estimates one week's minutes, not double", () => {
    const twoWeekQuest = quest("s1-w03-build");
    expect(twoWeekQuest.sittings).toBe(2);
    const summary = weekSummary([twoWeekQuest], {});
    expect(summary[0].minutesLeft).toBe(twoWeekQuest.minutes);
  });

  test("a one-week quest is unaffected by the sittings field", () => {
    expect(buildQuest.sittings).toBeUndefined();
    const summary = weekSummary([buildQuest], {});
    expect(summary[0].minutesLeft).toBe(buildQuest.minutes);
  });
});

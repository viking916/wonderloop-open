...
export type SeasonIndexTrackCell = {
  ...
  title: string;
  minutes: number;
  /** A heavy week's optional second sitting (23 September 2026); minutes stays the per-sitting
   * figure. Absent means the ordinary one sitting. */
  sittings?: 1 | 2;
  ...
  ideas: string[];
  /** True when this quest carries a monster problem-set (lane "monster"): season 1 authors one
   * in weeks 4, 8 and 11, one per sprint. A1 (2026-09-24): the monster is visible on This Week
   * for its whole sprint and never holds the week up, so the Season plan and the week plan say so
   * right on this cell rather than leaving a parent to wonder why a "done" Think week still shows
   * an unanswered problem elsewhere. */
  hasMonster: boolean;
};

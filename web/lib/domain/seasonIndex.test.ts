...
describe("explorerSeasonIndex: tracks", () => {
  // 23 September 2026: Play stopped being a weekly quest (lib/domain/tracks.ts), so the Season
  // plan never shows a Play column either, playTrack on or off -- it simply drops Play, the same
  // as any other track a season never authors.
  test("never shows a Play column, playTrack on or off", () => {
    ...
    expect(withPlay.every((w) => w.tracks.every((t) => t.track !== "play"))).toBe(true);
  ...
  // A1 (2026-09-24): season 1 authors a monster problem-set in weeks 4, 8 and 11 only.
  test("hasMonster is true only for the weeks that actually carry a monster problem-set", () => {
    const weeks = explorerSeasonIndex(1, {}, [], undefined);
    const think = (w: number) => weeks[w - 1].tracks.find((t) => t.track === "think")!;
    expect(think(4).hasMonster).toBe(true);
    expect(think(8).hasMonster).toBe(true);
    expect(think(11).hasMonster).toBe(true);
    expect(think(1).hasMonster).toBe(false);
    expect(think(3).hasMonster).toBe(false);
  });
});

...
// is passed in.
//
// "The week" here is the trailing WEEK_WINDOW_MS ending at `now`, not the content week a profile
// is currently on (lib/domain/calendar.ts's currentWeekFromDone, which is bounded by finishing
// the week, not by a date). This module has no reason to know about seasons or a profile's
// startDate -- a rolling window means "this week" always means "recently", the same thing a
// parent reading on any given day would mean by it, without threading a completion-derived
// content week through a module that otherwise has nothing to do with the calendar. `quests` is
// the one exception: which quests belong to "this week" is a content-week fact the caller
// already resolves (lib/content/app-content.ts's getWeek) to render This Week's own cards, so it
// arrives already scoped rather than re-derived here.

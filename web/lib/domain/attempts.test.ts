...
// ProblemPlayer.tsx's "What kind of miss was that?" row (Knowing what you know, 7 September
// 2026) used to identify which attempt doc a chosen miss kind belongs to from a separate,
// in-session-only `lastAttemptId` state, set only once handleSubmit's own persistAttempt promise
// resolved. Two real defects came from that: it raced the very same attempts listener that
// already drives view.triesUsed (the listener's local echo of a just-written attempt can arrive
// before handleSubmit's own await chain reaches the line that recorded the id -- reproduced
// directly against the running app: the row was reliably absent for roughly 100ms after the hint
// panel first appeared, which a screenshot taken in that gap, e.g. verify:ui's frozen-clock
// capture, can catch permanently), and it was lost outright on a reload or a remount, since it
// lived only in component state. currentCycleFirstAttemptId fixes both by deriving the id from
// the very attempts list (with ids) that already drives triesUsed, scoped to the current cycle
// exactly as currentCycleProblemState scopes it.
describe("currentCycleFirstAttemptId", () => {
  test("returns the current cycle's first (earliest) attempt id when it is the only one", () => {
    const id = currentCycleFirstAttemptId([{ id: "attempt-1", at: T0 + 100 }], T0);
    expect(id).toBe("attempt-1");
  });

  test("an attempt from before firstSeenAt (a finished, earlier cycle) is not picked", () => {
    const id = currentCycleFirstAttemptId([{ id: "old-cycle-attempt", at: T0 - 1 }], T0);
    expect(id).toBeUndefined();
  });

  test("with no attempts on record yet, there is nothing to return", () => {
    expect(currentCycleFirstAttemptId([], T0)).toBeUndefined();
  });

  test("picks the earliest in-cycle attempt regardless of list order (the first try, not the most recent one)", () => {
    const id = currentCycleFirstAttemptId(
      [
        { id: "try-2", at: T0 + 200 },
        { id: "try-1", at: T0 + 100 },
      ],
    ...
    expect(id).toBe("try-1");
  });

  test("a reload/remount rebuilds the same id from the same persisted attempts, with no in-session state involved", () => {
    // Simulates watchAttemptsForQuest re-delivering the same persisted attempt to a freshly
    // mounted component after a reload -- the id comes out the same as it did the first time,
    // because it is derived from Firestore-backed data, not from a value that only ever lived in
    // the component that submitted the attempt.
    const attemptsFromFreshSubscription = [{ id: "attempt-1", at: T0 + 100 }];
    const beforeReload = currentCycleFirstAttemptId(attemptsFromFreshSubscription, T0);
    const afterReload = currentCycleFirstAttemptId(attemptsFromFreshSubscription, T0);
    expect(beforeReload).toBe("attempt-1");
    expect(afterReload).toBe("attempt-1");
  });
});

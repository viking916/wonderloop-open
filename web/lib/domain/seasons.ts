...
/**
 * The "YYYY-MM-DD" (local) Monday on or before `now`: households.ts's startClock uses this as
 * the startDate a profile gets on its first saved step or round. A week no longer counts from
 * this date (owner decision, 13 September 2026: a week is bounded by finishing it, not by a
 * date -- lib/domain/calendar.ts's currentWeekFromDone); startDate stays a Monday-rounded record
 * of when the profile first began, kept for its own sake, not read to decide the current week.
 */

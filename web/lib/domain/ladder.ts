...
// design.md; the week's shape stopped being bound to weekdays on 13 September 2026, see that
// spec's dated addendum): sessions of rounds, three sessions to a week, finished in order rather
// than tied to which day it is; placement by probes; a topic at a time until the mastery bar; a
// fluency round; then retrieval on a schedule counted in sessions. No Firebase, no React, no
// Date.now() inside functions: time and randomness always arrive as parameters.
...
export type LadderState = {
   ...
   * `breakUntil` (added 15 September 2026, "skip the break") is the timestamp the break between
   * this round and the next is enforced until -- present only while a break is genuinely open,
   * never written as an explicit `undefined` (see `endRound`/`skipBreak` in LadderSession.tsx and
   * this repo's own history of a Firestore write with an explicit `undefined` field throwing
   * before the break screen could show at all). Skipping the break does not delete this field; it
   * collapses it to "now", which `breakActive` below already treats as over -- one field, one
   * pure read, for both the running countdown and a reload mid-break. */
  openSession?: { number: number; day: string; round: number; startedAt: number; breakUntil?: number };
};

/** Whether the break after a round is still enforced at `now`: it exists and has not yet been
 * reached, whether by the clock or by "Skip the break" collapsing it to the current time. Pure,
 * so a reload mid-break (which re-reads `LadderState.openSession.breakUntil` from Firestore
 * rather than any client-only timer) and the running countdown both ask the exact same question.
 * `undefined` means no break is open at all (never started, or a session that finished outright). */
export function breakActive(breakUntil: number | undefined, now: number): boolean {
  return breakUntil !== undefined && now < breakUntil;
}
...
/** The week on the Ladder (13 September 2026, replacing the old weekday-bound `roundsForDay`):
 * a plan of three sessions, one short and two long (the owner's five hours a week), finished in
 * order rather than bound to Friday, Saturday and Sunday. A week waits for the child instead of
 * expiring: nothing here reads a clock or a calendar date. */
export const WEEK_SESSION_ROUNDS = [2, 4, 4] as const;
export const SESSIONS_PER_WEEK = WEEK_SESSION_ROUNDS.length;
...
export type SessionStatus = "done" | "open" | "todo";
export type WeekEntry = { indexInWeek: number; rounds: number; roundsDone: number; status: SessionStatus };
export type LadderWeekPlan = {
  weekNumber: number;
  entries: WeekEntry[];
  next: { indexInWeek: number; rounds: number; roundsDone: number } | undefined;
  weekComplete: boolean;
};

/** The shape `ladderWeek` needs from one stored session record. Structurally compatible with
 * `lib/data/ladder.ts`'s `SessionDoc` (which carries more fields this pure helper does not need,
 * `day`, `startedAt`, `minutes`), so that type can be passed straight in; the domain layer never
 * imports from the data layer, even for a type, so this is declared here instead. */
export type LadderSessionRecord = {
  number: number;
  rounds: number;
  roundsDone?: number;
  endedAt?: number;
...
/**
 * The week on the Ladder as a plan of three sessions, finished in order, that waits instead of
 * expiring (13 September 2026, the owner's report: "weeks should not be bounded by dates but
 * should be bounded by completion of week"). A week is complete only once every one of its three
 * sessions is complete; nothing here reads a clock. The session already begun and not yet
 * finished (an "open" one) is always what comes next, even if some later slot in the week somehow
 * already carries a record too, so an unfinished session is never skipped for a later one.
 *
 * A session written before this change carries no `roundsDone`, `weekNumber` or `indexInWeek`:
 * one with an `endedAt` is treated as complete (`roundsDone` = its `rounds`), one without as
 * untouched (`roundsDone` 0, still open, never lost). Such a record's week and position are
 * inferred positionally, in session-number order, three to a week -- which lines up with the
 * owner's actual Friday/Saturday/Sunday rhythm this replaces (2, 4, 4 rounds, the same shape as
 * `WEEK_SESSION_ROUNDS`) unless a weekday taster session was ever opened in between, in which
 * case the inferred grouping can drift; this is a known limitation of reading old data, not a
 * silent one.
 */
export function ladderWeek(sessions: LadderSessionRecord[]): LadderWeekPlan {
  const sorted = [...sessions].sort((a, b) => a.number - b.number);
  const byWeek = new Map<number, WeekEntry[]>();
  let weekNumber = 1;
  let indexInWeek = 0;
  for (const s of sorted) {
    const roundsDone = s.roundsDone !== undefined ? s.roundsDone : s.endedAt !== undefined ? s.rounds : 0;
    const wn = s.weekNumber ?? weekNumber;
    const idx = s.indexInWeek ?? indexInWeek;
    const status: SessionStatus = roundsDone >= s.rounds ? "done" : "open";
    const list = byWeek.get(wn) ?? [];
    list[idx] = { indexInWeek: idx, rounds: s.rounds, roundsDone, status };
    byWeek.set(wn, list);
    // The running pointer only matters for the next record that has no weekNumber/indexInWeek of
    // its own (a legacy document); a record that names its own slot is never re-placed by it.
    if (status === "done") {
      indexInWeek = idx + 1;
      weekNumber = wn;
      if (indexInWeek >= SESSIONS_PER_WEEK) { indexInWeek = 0; weekNumber = wn + 1; }
    } else {
      indexInWeek = idx;
      weekNumber = wn;
    }
  }
  const latestWeek = byWeek.size ? Math.max(...byWeek.keys()) : 1;
  const found = byWeek.get(latestWeek) ?? [];
  const entries: WeekEntry[] = WEEK_SESSION_ROUNDS.map((rounds, i) => found[i] ?? { indexInWeek: i, rounds, roundsDone: 0, status: "todo" as const });
  const open = entries.find((e) => e.status === "open");
  const todo = entries.find((e) => e.status === "todo");
  const next = open ?? todo;
  const weekComplete = entries.every((e) => e.status === "done");
  return {
    weekNumber: latestWeek,
    entries,
    next: next ? { indexInWeek: next.indexInWeek, rounds: next.rounds, roundsDone: next.roundsDone } : undefined,
    weekComplete,
  };
}
...
/** The six spine stages' human titles (the grade band each sits at in a normal school is in
 * the design doc, never shown on a child's screen: principle 3, "placement, not grade"). Shared
 * by the parent view's map and the Explorer's Ladder card so the two screens never drift. */
export const STAGE_TITLES: Record<number, string> = {
  1: "Arithmetic",
  2: "Prealgebra",
  3: "Algebra 1",
  4: "Geometry",
  5: "Algebra 2 and precalculus",
  6: "Calculus",
};

export type StageStatus = "mastered" | "current" | "ahead";

export type StageSummaryEntry = { stage: number; title: string; total: number; mastered: number; status: StageStatus };

/** The stage the child is "at" for the "current" marker: the current topic's stage when one is
 * set; otherwise, before placement is done, the stage of any topic already placed or current
 * (a probe result); otherwise the first stage in spine order, so a family that has never opened
 * the Ladder still sees where it starts. */
function anchorStage(graph: LadderTopicMeta[], states: Record<string, TopicState>, currentTopic: string | undefined, stages: number[]): number | undefined {
  if (currentTopic) {
    const found = graph.find((t) => t.id === currentTopic);
    if (found) return found.stage;
  }
  const probed = orderedTopics(graph).find((t) => states[t.id]?.state === "placed" || states[t.id]?.state === "current");
  if (probed) return probed.stage;
  return stages[0];
}

/** One row per stage in spine order: how many of its topics are mastered, and whether the whole
 * stage is mastered, holds the child's current place, or is still ahead. Pure and read-only; it
 * never infers a topic's state, only summarises `states` as given. */
export function stageSummary(graph: LadderTopicMeta[], states: Record<string, TopicState>, currentTopic?: string): StageSummaryEntry[] {
  const stages = [...new Set(graph.map((t) => t.stage))].sort((a, b) => a - b);
  const at = anchorStage(graph, states, currentTopic, stages);
  return stages.map((stage) => {
    const topics = graph.filter((t) => t.stage === stage);
    const mastered = topics.filter((t) => states[t.id]?.state === "mastered").length;
    const allDone = topics.length > 0 && mastered === topics.length;
    const status: StageStatus = allDone ? "mastered" : stage === at ? "current" : "ahead";
    return { stage, title: STAGE_TITLES[stage] ?? `Stage ${stage}`, total: topics.length, mastered, status };
  });
}

...
export type StructuredRequest<T> = {
  ...
  maxTokens?: number;
  /** Defaults to MODEL (claude-opus-5). The tutor route (lib/ai/tutor.ts) asks for
   * claude-sonnet-5 instead: a Socratic hint or a two-sentence explanation is a much smaller
   * job than the debate opponent's argument, and every other caller keeps the default by

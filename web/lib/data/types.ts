...
import { builderChatId } from "../domain/builderAsk";
...
export type ProfileDoc = {
   ...
   * existing profile changed when the track arrived. Since 23 September 2026 this no longer
   * gates a weekly quest (see lib/domain/tracks.ts's header comment) -- it now only decides
   * whether the profile gets the piano row on the Practice this week card
   * (lib/domain/practice.ts). */
  playTrack?: boolean;
  /** Chess practice (23 September 2026, owner's ask): on by default for every Explorer profile,
   * so absent or true both mean on; only `false` turns it off. A parent flips this in the Parent
   * view for a profile that does not want the two weekend chess checkboxes on the Practice this
   * week card. See lib/domain/practice.ts. */
  chessPractice?: boolean;
  /** "YYYY-MM-DD": the day a parent paused this profile's week clock, back when the week
   * advanced with the calendar. Since 13 September 2026 a week ends when its quests are done,
   * so nothing advances on its own and nothing reads this field; it is kept so existing
   * profile documents stay valid and the history is not rewritten. */
  pausedAt?: string;
};
...
export type TutorRole = "child" | "tutor";
...
/**
 * households/{hid}/profiles/{pid}/tutorChats/{questId}__{problemId} (12 September 2026, "Ask":
 * a Socratic AI tutor a child can open when stuck on a problem, beyond the authored hints).
 * One document per problem this child has ever opened Ask on, keyed by questId and problemId
 * so it survives a reload and shows again in the Parent view (components/parent/AskTranscripts.tsx).
 * Never scored, never read by anything that scores an attempt -- the same purity as
 * WorkingDoc above. `prompt` is kept alongside the transcript so the Parent view can show what
 * the problem asked without a separate content lookup by id.
 *
 * Since 25 September 2026 (A4, Build and Make Ask, lib/domain/builderAsk.ts) the same collection
 * also holds one document per Build or Make STEP a child has opened Builder Ask on, at
 * builderChatId(questId, stepId) (a "__step__" id, distinct from a problem chat's plain
 * "__" join -- see that function's own comment). `kind: "step"` marks these; `problemId` is set
 * to the step's own id (there is no real problem id to carry) and `prompt` carries the step's
 * title, so every existing reader (AskTranscripts' trimPrompt(chat.prompt), watchTutorChats'
 * single query over the whole collection) already works unchanged. `kind` is absent on every
 * document written before this date, which all mean "problem" -- the original, and still the
 * more common, shape.
 */
export type TutorChatDoc = {
  questId: string;
  stepId: string;
  problemId: string;
  prompt: string;
  /** Absent means "problem" (every doc from before 25 September 2026, and every Ask problem
   * chat since). "step" marks a Builder Ask step chat. */
  kind?: "problem" | "step";
...
/**
 * households/{hid}/profiles/{pid}/practice/{docId} (23 September 2026: piano and chess practice
 * checkboxes -- see lib/domain/practice.ts for docId's shape, which rows a profile gets, and the
 * one-line summary). `docId` is `s{season}w{week}`, e.g. "s1w03", one document per profile per
 * week. Every field is optional and defaults to unticked; a document need not exist at all until
 * the first box is ticked. Ticking these never gates a week -- a week still ends when its quests
 * are done (owner ruling, 13 September 2026) -- so nothing that computes a profile's current week
 * (currentWeekFromDone, computeDoneByWeek) reads this collection.
 */
...
export const tutorChatConverter = converterWithTimestamps<TutorChatDoc>(["updatedAt"]);
export const practiceConverter = converterWithTimestamps<PracticeDoc>(["updatedAt"]);
...
export const tutorChatsPath = (hid: string, pid: string) => `households/${hid}/profiles/${pid}/tutorChats`;
export const tutorChatId = (questId: string, problemId: string) => `${questId}__${problemId}`;
export const tutorChatDocPath = (hid: string, pid: string, questId: string, problemId: string) =>
  `${tutorChatsPath(hid, pid)}/${tutorChatId(questId, problemId)}`;
/** Builder Ask's step-chat id, delegated to lib/domain/builderAsk.ts's builderChatId so the id
 * shape is defined and tested in exactly one place. */
export const tutorChatStepDocPath = (hid: string, pid: string, questId: string, stepId: string) =>
  `${tutorChatsPath(hid, pid)}/${builderChatId(questId, stepId)}`;
export const practicePath = (hid: string, pid: string) => `households/${hid}/profiles/${pid}/practice`;
export const practiceDocPath = (hid: string, pid: string, docId: string) => `${practicePath(hid, pid)}/${docId}`;
...
export const tutorChatsCol = (db: Firestore, hid: string, pid: string): CollectionReference<TutorChatDoc> =>
  collection(db, tutorChatsPath(hid, pid)).withConverter(tutorChatConverter);
export const tutorChatRef = (
  db: Firestore, hid: string, pid: string, questId: string, problemId: string,
): DocumentReference<TutorChatDoc> => doc(db, tutorChatDocPath(hid, pid, questId, problemId)).withConverter(tutorChatConverter);
export const tutorChatStepRef = (
  db: Firestore, hid: string, pid: string, questId: string, stepId: string,
): DocumentReference<TutorChatDoc> => doc(db, tutorChatStepDocPath(hid, pid, questId, stepId)).withConverter(tutorChatConverter);
export const practiceCol = (db: Firestore, hid: string, pid: string): CollectionReference<PracticeDoc> =>
  collection(db, practicePath(hid, pid)).withConverter(practiceConverter);
export const practiceRef = (db: Firestore, hid: string, pid: string, docId: string): DocumentReference<PracticeDoc> =>
  doc(db, practiceDocPath(hid, pid, docId)).withConverter(practiceConverter);

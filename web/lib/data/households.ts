 ...
 * Since 23 September 2026 this only gates the piano checkbox on the Practice this week card
 * (lib/domain/practice.ts), not a weekly quest. */
...
/** Turns chess practice on or off for one Explorer profile (23 September 2026); nothing else
 * changes. On by default (absent or true), so this is only ever called to turn it off, or to
 * turn a profile that was explicitly turned off back on. */
export async function setChessPractice(hid: string, pid: string, on: boolean): Promise<void> {
  const db = getDb();
  await updateDoc(profileRef(db, hid, pid), { chessPractice: on });
}

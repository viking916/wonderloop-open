...
/** Every screenTime/{yyyy-mm-dd} doc this profile has, unfiltered -- the Parent view slices this
 * itself against a real calendar week (lib/domain/calendar.ts's calendarWeekRange; a content
 * week no longer maps to any fixed set of real dates, so there is no single "this content week's
 * dates" to filter by here). A whole season is at most 84 days, so one small collection watch is
 * simpler and just as live as a per-week ranged query. */

...
// `day` keeps recording the real weekday a session ran on -- a fact for the record, never
// something read back to decide anything (see lib/domain/ladder.ts's `ladderWeek`, which is the
// only place that decides where a child is in the Ladder week). `roundsDone`, `weekNumber` and
// `indexInWeek` are new as of 13 September 2026; a document written before that date has none of
// the three, which `ladderWeek` treats as a legacy record (see its own doc comment).

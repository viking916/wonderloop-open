...
export const QuestSchema = z.object({
  ...
  /** A heavy quest that runs over more than one calendar week at the same weekly pace (owner
   * approved, 23 September 2026; meaning corrected the same day). minutes stays the per-week
   * figure (still checked against TRACK_MINUTES below) and is NOT multiplied by sittings: a
   * sittings: 2 quest's weekly time estimate is the same as an ordinary one-week quest's, since
   * this field says how many calendar weeks the quest takes, never how many minutes. Shown
   * wherever a quest's minutes reach the child or the parent via lib/domain/questTime.ts's
   * questTimeLabel, e.g. "2 weeks, about 60 min each". */
  sittings: z.union([z.literal(1), z.literal(2)]).optional(),

...
kind, at ten viewports (1280x900, 820x1180, 1194x834, 390x844, 834x1194, 1024x1366, 1536x864,
1920x1080, and, added 23 September 2026 so a laptop-width breakpoint tuned to a round number like
1280 or 1400 cannot quietly miss the actual laptop widths Windows display scaling reports,
1280x720 and 1366x768) -- including all 12 of the season's weeks on "This Week" (via the
...
**What's covered.** Fifteen screens, at eight of the sweep's ten viewports (120 baselines
committed in `docs/visual-baselines/` as of 23 September 2026). The two viewports added that day,
1280x720 and 1366x768, run every other check above but do not have baselines yet -- their first
`--update-baselines` accept, after a deliberate look at their own fresh screenshots, is still
open work. The fifteen screens (`VISUAL_TARGETS` in the script): the profile picker,
...
the Sprout hill, a Sprout activity, a Sprout pile round (`activity-piles`, since a regression that
flattens a pile back to one glyph is invisible to every other check), the parent PIN gate
(`parent-view`) plus its This week/Season plan/Settings tabs (`parent-week`, `parent-plan`,
`parent-settings`), and the style gallery. Every *other*
...
flagged something) *before* running this. It overwrites every one of the committed baselines
...
- **The browser clock is frozen**, not the wall clock: `context.clock.setFixedTime()` (Playwright's
  ...
  reseed rather than needing a hand-maintained constant). Every `checkCurrentPage` call
  ...
  more surprising reason (see the next bullet). **Stale as of 13 September 2026:** the current
  week used to come from this same frozen `startDate` via `currentWeekFor`, so freezing the clock
  was also what kept the gate's route table landing on week 1; the owner's completion-only week
  rule replaced that function with `currentWeekFromDone` (lib/domain/calendar.ts), so the route
  table now lands on week 1 because the seeded household's own progress leaves week 1 not done,
  not because of the frozen clock. Freezing the clock still matters for everything else on this
  list (log and attempt timestamps, cooldowns, "About N min" labels); this gate itself was not
  re-run or updated as part of that change, so its comments (`computeFrozenNow` and nearby) still
  describe the old mechanism and want a look before the next baseline update.

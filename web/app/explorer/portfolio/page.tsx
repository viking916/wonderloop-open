  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <main className="mx-auto w-full max-w-[1080px] px-4 py-6 sm:px-6">
        {/* Every one of the five watchers above feeds either entries or badgeProgress, so
            rendering before allLoaded (the same signal the badge-celebration effect already
            waits on) painted a real "Nothing in the journal yet" for the split second before
            Firestore's first snapshot arrived -- true on a slow connection, not just under the
            heavy concurrent emulator load that first caught it here. Matches the skills map's
            own allLoaded gate. */}
          ...
          <>
            ...
            <Card tone="surface" shadow className="!p-0 tr-portfolio">
              <div className="tr-journal">
                <div className="tr-journal__head">
                  <h2 className="tr-page-title">Portfolio</h2>
                  {entries.length > 0 ? (

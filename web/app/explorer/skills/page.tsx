...
import { TRACK_LABEL, TRACK_ORDER } from "@/lib/content/schema";
...
function SkillsHome({ profile, householdId }: { profile: Profile; householdId: string }) {
  ...
  const tracksWithSkills = useMemo(() => {
    // The skills map is not the weekly quest flow -- tracksForProfile (lib/domain/tracks.ts)
    // never returns "play" any more, since Play stopped being a quest on 23 September 2026. This
    // page still shows Play skills whenever the profile has the Play track on OR has real
    // evidence on record (so turning the track off later never hides something already earned),
    // so it reads TRACK_ORDER directly instead.
    const hasPlayEvidence = skillDocs.some((s) => s.skillId.startsWith("play."));
    const showPlay = profile.playTrack === true || hasPlayEvidence;
    const tracks = TRACK_ORDER.filter((t) => t !== "play" || showPlay);

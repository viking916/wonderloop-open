...
import { SideStrip } from "@/components/explorer/SideStrip";
import { TalkOutlineCard } from "@/components/explorer/TalkOutlineCard";
import { LadderCard } from "@/components/explorer/LadderCard";
import { PracticeCard } from "@/components/explorer/PracticeCard";
...
import { findSprintMonster } from "@/lib/domain/monster";
...
/**
 * Which of the twelve weeks are done for this profile (owner decision, 13 September 2026: a
 * week is bounded by finishing it, not by a date): every track this profile has, on that week,
 * at questStatus "done". A week with none of this profile's quests authored for it counts as
 * not done, the same as a week nothing has been started on -- there is nothing to have finished.
 * Feeds both currentWeekFromDone (lib/domain/calendar.ts) and buildTrailWeeks below, so the
 * trail's done/current marks and the real current week can never disagree with each other.
 */
function computeDoneByWeek(seasonId: number, progressDocs: Record<string, ProgressDoc>, choice: TrackChoice): boolean[] {
...
/**
 * Builds the twelve-week SeasonTrail model: done/current/todo per week (done straight from
 * `doneByWeek`, computeDoneByWeek's own result, so this never re-derives it), plus a flag (the
 * Build quest's title and the latest completion date) on any Showcase week that is fully done.
 * "current" always marks the real current week, never the picker's displayed week, so jumping
 * the picker never moves the "you are here" marker.
 */
function buildTrailWeeks(
  ...
  for (let week = 1; week <= 12; week++) {
    ...
    const done = doneByWeek[week - 1] ?? false;
...
function ExplorerHome({ profile, householdId }: { profile: Profile; householdId: string }) {
  ...
  // September 2026): computeDoneByWeek reads the same questStatus the trail and the track cards
  // already show, so "current" can never disagree with what the child actually finished.
  const doneByWeek = useMemo(() => computeDoneByWeek(seasonId, progressDocs, profile), [seasonId, progressDocs, profile]);
  const currentWeek = useMemo(() => currentWeekFromDone(doneByWeek), [doneByWeek]);

  // The picker's displayed week (task 8 brief). `progressDocs` starts empty and the real
  // currentWeek only becomes known once the first Firestore snapshot arrives, so a fresh
  // session with no stored pick follows currentWeek as it resolves rather than freezing on
  // whatever the empty-progress guess was; once the picker actually jumps, `pickedRef` stops
  // that from ever overriding the parent's own choice again this session.
  const pickedRef = useRef(readStoredWeek(profile.id) !== undefined);
  const [displayedWeek, setDisplayedWeekState] = useState<number>(() => readStoredWeek(profile.id) ?? currentWeek);
  useEffect(() => {
    if (!pickedRef.current) setDisplayedWeekState(currentWeek);
  }, [currentWeek]);

  const pickWeek = useCallback(
    (week: number) => {
      pickedRef.current = true;
      setDisplayedWeekState(week);
  ...
  const trailWeeks = useMemo(
    () => buildTrailWeeks(seasonId, currentWeek, doneByWeek, progressDocs, profile),
    [seasonId, currentWeek, doneByWeek, progressDocs, profile],
  );
  // findSprintMonster wants QuestProgress per quest, not the whole ProgressDoc; progressDocs
  // already carries every quest's progress (watchProgress watches the whole collection, not just
  // this week's), so this is a cheap reshape, not a second Firestore read.
  const progressByQuest = useMemo(
    () => Object.fromEntries(Object.entries(progressDocs).map(([id, doc]) => [id, doc.quest])),
    [progressDocs],
  );
  const monster = useMemo(
    () => findSprintMonster(seasonId, displayedWeek, progressByQuest),
    [seasonId, displayedWeek, progressByQuest],
  );
  ...
  // Returns the write's own Promise (not fire-and-forget) so SideStrip's "We played it" Button
  // can auto-track it as pending (owner report: buttons that do not respond instantly read as
  // broken) -- see components/ui/Button.tsx's Promise-tracking doc comment. The .catch still
  // swallows the error here rather than letting it reach Button's pending machinery as a
  // rejection to handle, since a failed "mark as played" write has nothing more useful to show
  // than the card simply staying on its own "We played it" button for another try.
  const onGamePlayed = useCallback((gameId: string): Promise<void> => {
    return markGamePlayed(householdId, profile.id, gameId, Date.now()).catch(() => undefined);
  }, [householdId, profile.id]);
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="tr-page">
        <Card tone="surface" shadow className="!p-0 tr-this-week">
          ...
          <LadderCard householdId={householdId} profileId={profile.id} now={now} />
          <PracticeCard householdId={householdId} profileId={profile.id} seasonId={seasonId} week={displayedWeek} choice={profile} />

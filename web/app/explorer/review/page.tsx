...
function ReviewHome({ profile, householdId }: { profile: Profile; householdId: string }) {
  ...
  // Aside stats (laptop-width polish, 15 September 2026): the same dueItems the batch itself is
  // built from, recomputed here rather than threaded through state, so the side column can never
  // disagree with what the main column is actually showing.
  const dueNowCount = useMemo(() => dueItems(reviewQueue, now).length, [reviewQueue, now]);
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="tr-page">
        <div className="rv-layout">
          <Card tone="surface" shadow className="!p-0 rv-main">
            <div className="tr-quest__main">
              <p className="tr-eyebrow">Mistake box</p>
              <h2 className="tr-page-title">Second look</h2>
              ...
              {!queueLoaded || !progressLoaded || batch === null ? null : batch.length === 0 ? (
                ...
                <>
                  {/* Only shown here, mid-batch: the two states above already end in their own
                      primary "Back to This week" (EmptyState's action), so a second, quiet copy of
                      the same label right above it read as two controls saying one thing. This is
                      the one state with no other way home, since ProblemPlayer offers no home link
                      of its own. */}
          ...
          <aside className="rv-aside">
            {/* Round 2 fix (15 September 2026): tone="kraft" backed these panels with the exact
                same tan as the page itself, and .tr-card--kraft carries no border, so they read
                as plain floating text with no card at all -- unlike every other card on this
                screen (or anywhere else in the app). tone="surface" gives them the same white,
                bordered treatment as the main column. */}
            <Card tone="surface" className="rv-aside__card">
              <h3>What is the mistake box</h3>
              <p>
                A wrong first try on a problem comes back here later, in a new shape, for one more
                look. Nothing is graded twice; this is just another chance at the same idea.
              </p>
            </Card>
            <Card tone="surface" className="rv-aside__card">
              <h3>What comes back when</h3>
              <div className="rv-aside__stats">
                <div className="rv-aside__stat">
                ...
                <div className="rv-aside__stat">
                  <span>Next one back</span>
                  <strong>{nextUpcoming ? formatDate(nextUpcoming.dueAt) : "Nothing waiting"}</strong>
                </div>
              </div>
              <p>
                Up to {MAX_REVIEW_BATCH} at a time; anything past that waits and this page picks it
                up again next visit.
              </p>
            </Card>
          </aside>

...
export default function StyleGalleryPage() {
  ...
  return (
    <div>
      ...
      <Section label="Waiting for something to finish">
        <p style={{ color: "var(--muted)", marginTop: 0, marginBottom: 16, fontSize: 14, maxWidth: 680 }}>
          Owner report: &quot;Many times when I press button nothing happens instantly so it makes
          me feel like something is broken.&quot; Every one of these waits out about 120ms before
          showing anything at all, and once shown stays for about 400ms more, so a fast tap never
          strobes a spinner on and off (lib/pendingTiming.ts&apos;s PENDING_SHOW_DELAY_MS and
          PENDING_MIN_VISIBLE_MS). None of the tiers below go grey to say this: a pending Primary
          keeps its own blaze fill, a pending Destructive keeps its double border and warning icon
          -- the washed-out disabled fill is reserved for a control that truly cannot be pressed,
          never for one that is already working.
        </p>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="secondary" href="#" pending>
            Start
          </Button>
        </div>
        <dl style={{ marginTop: 14, marginBottom: 0, display: "grid", gap: 8, fontSize: 14, color: "var(--muted)" }}>
          <div>
            <strong style={{ color: "var(--forest)" }}>Primary pending</strong> -- a Check, Save or
            Continue that is actually writing something: pass nothing extra at all if the onClick
            you already have returns a Promise, Button.tsx tracks it for you.
          </div>
          <div>
            <strong style={{ color: "var(--forest)" }}>Secondary pending</strong> -- a real
            dashboard action mid-flight (Open as Explorer while its own onClick is still
            resolving, Rename while it saves).
          </div>
          <div>
            <strong style={{ color: "var(--forest)" }}>Quiet pending</strong> -- rare (most Quiet
            actions are instant, like Back), but the same treatment applies if one ever does
            genuine work, so it never needs its own special case.
          </div>
          <div>
            <strong style={{ color: "var(--blaze-deep)" }}>Destructive pending</strong> -- a reset
            actually running. Keeps its double border and warning icon throughout: a moment that
            already asks for care should never look like it stopped responding.
          </div>
          <div>
            <strong style={{ color: "var(--forest)" }}>An anchor (href) pending</strong> -- a
            navigation link whose &quot;pending&quot; reason is not its own onClick (gated on
            something else finishing first). Pass the `pending` prop explicitly; Button.tsx blocks
            the click outright while it is true, the same as the button forms above.
          </div>
        </dl>

        <p style={{ color: "var(--muted)", marginTop: 20, marginBottom: 8, fontSize: 14, maxWidth: 680 }}>
          Not a Button at all -- a form submit, a file input, a card press, a select&apos;s
          onChange -- gets the identical timing from <code>useAsyncAction</code>
          (lib/useAsyncAction.ts): <code>const {"{ run, pending, error }"} = useAsyncAction(save)</code>,
          then drive whatever that control&apos;s own busy look is from `pending`, and its own
          error message from `error`.
        </p>

        <p style={{ color: "var(--muted)", marginTop: 20, marginBottom: 8, fontSize: 14, maxWidth: 680 }}>
          Loading a whole new screen (Start, Open the Ladder, Back to This week, Open as Explorer)
          used to look identically dead while the next page loaded. A thin bar now runs across the
          very top of the viewport for exactly that window -- mounted once, in the root layout, so
          no screen has to add it for itself:
        </p>
        <div
          className="tr-route-progress"
          data-visible="true"
          style={{ position: "relative", opacity: 1, top: "auto", left: "auto", right: "auto", maxWidth: 420 }}
        >
          <i />
        </div>
        <p style={{ color: "var(--muted)", marginTop: 8, marginBottom: 0, fontSize: 14 }}>
          Blaze on kraft, about 3px, aria-hidden decoration with no layout shift of its own (it is
          always `position: fixed`, never part of the flow). It only ever appears for a
          navigation slower than the same ~120ms rule as the buttons above, and never for one that
          lands instantly.
        </p>
      </Section>
      ...
      <Section label="Learn it lessons: headings and callouts (docs/content-authoring.md rule 20)">
        <p style={{ color: "var(--muted)", marginTop: 0, marginBottom: 16, fontSize: 14, maxWidth: 680 }}>
          A &quot;Learn it&quot; lesson (rule 32) used to be one long wall of prose: every
          paragraph the same weight, a trick or a common trap buried in the middle of a sentence.
          Two new plain-text conventions, added to the same body a lesson already writes (no
          markdown, no new field): a single line ending in a colon that names a known lesson
          section becomes a small heading, and a paragraph starting <code>Trick:</code>,{" "}
          <code>Trap:</code>, <code>Check it:</code> or <code>Remember:</code> becomes a callout
          box with a real label, not colour alone. A body written before either convention existed
          renders exactly as before -- these only fire on the exact marker text (StepBody.test.tsx,
          StepBody.contentBundle.test.tsx).
        </p>
        <StepBody
          text={
            "Your robot's battery holds 40 minutes of driving, and the screen says 3/4 is left. How many minutes can it still drive?\n\n" +
            "The idea:\n\n" +
            "A fraction has two numbers. The bottom number says how many equal parts to cut the whole into. The top number says how many of those parts you take.\n\n" +
            "Tricks:\n\n" +
            "Trick: `bottom, then top`. Divide by the bottom number first, then multiply by the top. Bottom first, every time.\n\n" +
            "Trap: dividing by the top number instead of the bottom. 3/4 of 40 is not 40 divided by 3.\n\n" +
            "Check it: does your answer make sense? A fraction less than 1 of an amount should always come out smaller than the amount you started with.\n\n" +
            "Worked example:\n\n" +
            "What is 2/3 of 18 LEGO bricks?\n\n" +
            "1. Bottom number first: 18 divided by 3 is 6. One third is 6 bricks.\n2. Top number next: 2 x 6 is 12.\n3. So 2/3 of 18 is 12 bricks.\n\n" +
            "Try one with me:\n\n" +
            "Cover the lines below and say what you think comes next before you look.\n\n" +
            "1. Bottom number: 20 divided by 4 is 5.\n2. Top number: 3 x 5 is 15.\n3. So 3/4 of 20 is 15.\n\n" +
            "Remember: 'of' means times, and one 'of' can follow another.\n\n" +
            "What to remember:\n\n" +
            "1. Bottom number: divide. Top number: multiply. Bottom first, every time.\n2. The trap: dividing by the top number.\n3. Check it against the amount you started with."
          }
        />
      </Section>
